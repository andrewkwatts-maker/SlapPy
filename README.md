...
- **Native Rust hot paths.** Renderer kernels, softbody/PBF inner loops, and the `_core` extension move performance-critical work out of Python — fluid/softbody scenes run at hundreds of fps with no engine-side allocation.
- **Unified dynamics primitives.** Components, collision, lighting, fluid, and animation all sit behind a single lazy-loaded `slappyengine` namespace so games can `import slappyengine as sle` and stop thinking about subpackage layout.
- **Generalised engine surface.** 75 top-level symbols + 13 declared subpackages form the v0.3 contract, documented in [`docs/engine_surface_v030.md`](docs/engine_surface_v030.md) (auto-generated by `scripts/gen_engine_surface_doc.py`).
