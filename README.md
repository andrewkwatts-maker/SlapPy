...
Every demo under [`examples/`](examples/) is runnable headlessly and
renders its final state to a PNG.  The grid below is regenerated by
`tools/run_examples.py`, which invokes every `examples/hello_*.py` with
`--render --frames 60 --out <tmp>/<demo>.png` and composes the results.
...
Demos that do not yet support `--render` (window-based hello-world
samples) show as red `FAILED` cells; the runner keeps going so the grid
always builds.
