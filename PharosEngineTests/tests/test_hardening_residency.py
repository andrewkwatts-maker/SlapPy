...
# Some platforms (CI without wgpu.gui) can't import pharos_engine.* eagerly.
...
def test_compress_array_accepts_ndarray():
    ...
    from pharos_engine.residency.compression import (
...
def test_decompress_raw_rejects_list():
    from pharos_engine.residency.compression import decompress_raw
    with pytest.raises(TypeError, match="data must be bytes-like"):
        decompress_raw([0, 1, 2])


def test_decompress_array_rejects_empty_shape():
    from pharos_engine.residency.compression import decompress_array
...
def test_write_asset_to_slap_rejects_none_asset(tmp_path):
    from pharos_engine.residency.slap_format import write_asset_to_slap
...
def test_read_asset_from_slap_rejects_none_path():
    from pharos_engine.residency.slap_format import read_asset_from_slap
    with pytest.raises(TypeError, match="path must be str or pathlib.Path"):
