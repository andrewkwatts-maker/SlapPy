...
# ---------------------------------------------------------------------------
# Detect whether Pharos Engine is importable (wgpu.gui may be absent in CI).
# Compression tests only use numpy, so they always run.
# Everything else is gated on this flag.
# ---------------------------------------------------------------------------
_ENGINE_SKIP_REASON = ""
try:
    import pharos_engine  # noqa: F401 — triggers __init__ (Engine import)
...
_requires_engine = pytest.mark.skipif(
    ...
    reason=f"Pharos Engine not importable: {_ENGINE_SKIP_REASON}",
)
...
class TestSlapFormat:
    ...
    def test_no_asset_raises(self, tmp_path):
        from pharos_engine.residency.slap_format import write_world_slap, read_asset_from_slap
