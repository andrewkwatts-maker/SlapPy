...
from pharos_editor.tool_router import ToolAction, ToolRouter
from pharos_editor.ui.hotkey_remap import HotkeyBinding, HotkeyMap
...
def _make_bar(**kwargs):
    from pharos_editor.ui.editor.notebook_menu_bar import NotebookMenuBar
...
class TestMenuOrder:
    def test_menu_order_is_canonical(self):
        from pharos_editor.ui.editor.notebook_menu_bar import MENU_ORDER
        assert MENU_ORDER == (
            "file", "edit", "view", "tool", "panel", "theme", "spawn", "help",
        )

    def test_category_glyphs_cover_menu_order(self):
        from pharos_editor.ui.editor.notebook_menu_bar import (
            CATEGORY_GLYPHS,
            MENU_ORDER,
        )
    ...
    def test_diary_glyph_values(self):
        from pharos_editor.ui.editor.notebook_menu_bar import CATEGORY_GLYPHS
...
class TestEditorRegistration:
    def test_lazy_import_via_editor_init(self):
        from pharos_editor.ui.editor import NotebookMenuBar
        bar = NotebookMenuBar()
        assert bar.router is None
...
class TestFormatShortcut:
    def test_none_returns_empty(self):
        from pharos_editor.ui.editor.notebook_menu_bar import format_shortcut
