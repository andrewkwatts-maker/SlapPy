...
class TestMeshVertex:
    def test_instantiates(self):
        from pharos_engine.gpu.mesh import MeshVertex
...
class TestGpuMesh:
    def test_instantiates(self):
        from pharos_engine.gpu.mesh import GpuMesh, MeshVertex
...
class TestCluster3DConstants:
    def test_tiles_x(self):
        from pharos_engine.gpu.cluster_3d import TILES_X
        assert TILES_X == 16

    def test_tiles_y(self):
        from pharos_engine.gpu.cluster_3d import TILES_Y
        assert TILES_Y == 9

    def test_tiles_z(self):
        from pharos_engine.gpu.cluster_3d import TILES_Z
        assert TILES_Z == 24

    def test_total_clusters(self):
        from pharos_engine.gpu.cluster_3d import TOTAL_CLUSTERS, TILES_X, TILES_Y, TILES_Z
        assert TOTAL_CLUSTERS == TILES_X * TILES_Y * TILES_Z

    def test_max_lights(self):
        from pharos_engine.gpu.cluster_3d import MAX_LIGHTS
        assert MAX_LIGHTS == 256

    def test_max_lights_per_cluster(self):
        from pharos_engine.gpu.cluster_3d import MAX_LIGHTS_PER_CLUSTER
        assert MAX_LIGHTS_PER_CLUSTER == 64

    def test_aabb_stride_32(self):
        from pharos_engine.gpu.cluster_3d import _AABB_STRIDE
        assert _AABB_STRIDE == 32

    def test_light_stride_32(self):
        from pharos_engine.gpu.cluster_3d import _LIGHT_STRIDE
