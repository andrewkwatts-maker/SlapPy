...
class TestEffectShader:
    def test_instantiates(self):
        from pharos_engine.compute.effect import EffectShader
...
class TestMutatorConstants:
    def test_filter_op_tag(self):
        from pharos_engine.compute.mutator import _FILTER_OP_TAG
        assert _FILTER_OP_TAG == 0

    def test_filter_op_gt(self):
        from pharos_engine.compute.mutator import _FILTER_OP_GT
        assert _FILTER_OP_GT == 1

    def test_filter_op_lt(self):
        from pharos_engine.compute.mutator import _FILTER_OP_LT
        assert _FILTER_OP_LT == 2

    def test_filter_op_eq(self):
        from pharos_engine.compute.mutator import _FILTER_OP_EQ
        assert _FILTER_OP_EQ == 3

    def test_constants_unique(self):
        from pharos_engine.compute.mutator import (
...
class TestSpatialComputeImport:
    def test_importable(self):
        from pharos_engine.compute.spatial import SpatialCompute
...
class TestAssetComputeImport:
    def test_importable(self):
        from pharos_engine.compute.asset_compute import AssetComputeAPI
        assert AssetComputeAPI is not None

    def test_stats_result_re_exported(self):
        from pharos_engine.compute.asset_compute import StatsResult
        assert StatsResult is not None

    def test_aabb_re_exported(self):
        from pharos_engine.compute.asset_compute import AABB
