...
Covers:
- pharos_engine.tags            (TagRegistry)
- pharos_engine.z_height        (ZLayer, ZAABBShape, ZHeightModule, check_z_aabb)
- pharos_engine.render_channel  (RenderPass, NightVisionPass, ThermalPass,
                                RenderChannelCompositor pure-Python API)
- pharos_engine.render_target   (RenderTarget)
"""
...
class TestRenderChannelCompositor:
    ...
    def test_add_channel_by_pass(self):
        from pharos_engine.render_channel import RenderChannelCompositor, RenderPass
        c = self._make_compositor()
