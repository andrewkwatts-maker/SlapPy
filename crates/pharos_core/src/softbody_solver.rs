...
/// `_pack_cell_keys` in `python/pharos_engine/softbody/collision.py`.
...
/// `python/pharos_engine/softbody/collision.py`:
...
/// `python/pharos_engine/softbody/collision.py`. The contact radius is
...
/// point (via thin slice-grab wrapper) and by `pharos_engine_step`.
#[allow(clippy::too_many_arguments)]
