...
// -- Sprint 4 SOLID refactor: SSoT for kernel constants --

pub mod config {
    //! Single source of truth for CSM pipeline compile-time constants.
    //! Every magic number the WGSL shader consults through a `const`
    //! binding lives here; [`wgsl_define_block`] emits the WGSL header
    //! prepended to `shadow_csm.wgsl` at pipeline creation.

    /// Number of cascades sampled per fragment. Nova3D shipped 4.
    /// Storage: `array<CsmCascade, 4>` in the shader — changing this
    /// requires a matching bind-group / struct edit.
    pub const CASCADE_COUNT: u32 = 4;

    /// Per-cascade PCF sample count. Vogel disk with 8 taps is the
    /// Sprint 8 default; 4 taps for Compat, 16 for Cinematic.
    pub const PCF_TAP_COUNT: u32 = 8;

    /// Emit the WGSL `const` header prepended to `shadow_csm.wgsl`.
    pub fn wgsl_define_block() -> String {
        format!(
            "// GENERATED by pharos_render::shadow::csm::config::wgsl_define_block\n\
             const CSM_CASCADE_COUNT: u32 = {cc}u;\n\
             const CSM_PCF_TAP_COUNT: u32 = {pc}u;\n",
            cc = CASCADE_COUNT,
            pc = PCF_TAP_COUNT,
        )
    }
