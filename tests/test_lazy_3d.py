...
def test_import_does_not_load_3d():
    """Importing playslap must NOT trigger 3D module imports."""
    # Remove any cached 3D modules so the check is fresh
    for key in list(sys.modules.keys()):
        if "mesh_pipeline" in key or "pbr_material" in key or "mesh" in key:
            del sys.modules[key]

    import playslap  # noqa: F401

    assert "playslap.gpu.mesh_pipeline" not in sys.modules
    assert "playslap.gpu.pbr_material" not in sys.modules
...
def test_gpu_mesh_vertex_stride():
    ...
    from playslap.gpu.mesh import GpuMesh, MeshVertex
