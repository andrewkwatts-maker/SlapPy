...
::
    ...
    python SlapPyEngineExamples/examples/hello_export_cli.py
...
def _make_synthetic_project(root: Path) -> Path:
    ...
    # The exporter loads slappyproject.yaml if present; we also drop a
    # minimal config.yaml so the manifest.name reads back sensibly.
    (proj / "slappyproject.yaml").write_text(
