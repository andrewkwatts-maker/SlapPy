...
struct MutateParams {
    ...
    stride_u32s:    u32,
    channel_offset: u32,
    value:          f32,     // delta to add
    filter_op:      u32,
    filter_ch_off:  u32,
    filter_value:   f32,
    clamp_output:   u32,     // 1 = clamp result to [0,1]
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
    ...
    if pass_filter {
        var cur = bitcast<f32>(pixels[base + params.channel_offset]);
        cur = cur + params.value;
        if params.clamp_output != 0u {
            cur = clamp(cur, 0.0, 1.0);
        }
        pixels[base + params.channel_offset] = bitcast<u32>(cur);
    }
}
