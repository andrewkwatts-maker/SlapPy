// brdf_lut.wgsl
// Pre-compute GGX split-sum BRDF LUT (512×512)
// Output: rg16float where R=scale (F_schlick integral), G=bias
// Run ONCE at startup. Based on Epic Games 2013 split-sum approximation.
//
// Bindings:
//   group(0) binding(0) — lut_out  texture_storage_2d<rg16float, write>

@group(0) @binding(0) var lut_out: texture_storage_2d<rg16float, write>;

const LUT_SIZE:     u32 = 512u;
...
// IBL geometry sub-function uses k = a²/2 (image-based, not direct lighting).
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    if (gid.x >= LUT_SIZE || gid.y >= LUT_SIZE) { return; }

    // x axis = N_dot_V in (0, 1], y axis = roughness in (0, 1]
    let n_dot_v   = (f32(gid.x) + 0.5) / f32(LUT_SIZE);
    let roughness = (f32(gid.y) + 0.5) / f32(LUT_SIZE);

    // Build a view vector with the given N·V angle in a tangent-space frame
    // where N = (0, 0, 1).
    ...
    textureStore(lut_out, vec2<i32>(gid.xy), vec4<f32>(scale, bias, 0.0, 1.0));
}
