...
* **WIRED**: 313 (215 baseline + 18 delta + 5 Y1 + 5 Z7 + 5 AA1 + 5 BB1 + 5 CC1 + 5 DD1 + 5 EE1 + 5 FF1 + 5 GG1 + 5 II5 + 5 JJ6 + 5 KK7 + 19 JJ+KK+LL rendering/capture landings; rows 189 + 243 also flipped STUB -> WIRED, row 50 flipped STUB -> WIRED under LL1 HUDOverlay)
* **STUB**: 13 (rows 78, 79, 94, 95, 191, 192, 193, 222, 224, 225, 226, 227, 228 — row 50 flipped to WIRED under LL1, row 243 flipped under GG1)
...
## MM3 JJ+KK+LL landings audit (2026-07-05, post-KK7)

Nineteen user-invocable capabilities landed during the JJ (Nova3D
parity sprints 1-7), KK (sprints 6-13), and LL (sprints 14-20)
batches — 3D renderer, glTF/OBJ importers, animation runtime,
lighting/post passes, HUD, capture, audio, and shipping surfaces.
Rows 312–330 capture every landing that isn't already tracked by an
earlier row. Each cross-references its landing commit in the git
log and a real Python file:line in the repo.

Categories represented (row → sprint):

* **Renderer core (JJ)**: real wgpu forward pipeline (312/JJ1),
  MTL resolver (313/JJ2), skinned glTF (314/JJ3), skeletal
  animation + CSM shadows (315/JJ4 + 317/JJ7), scene walker
  (316/JJ5).
* **Renderer polish (KK)**: BVH broadphase (318/KK1), pass chain
  + MSAA (319/KK2), SSAO (320/KK3), skybox (321/KK4), IBL
  prefilter (322/KK5), SDF text (323/KK6).
* **Ship-a-game (LL)**: HUD overlay (325/LL1), video capture
  (326/LL2), instanced rendering (324/LL3), 3D audio (327/LL4),
  glTF character demo (328/LL5), game exporter (329/LL6), 3D
  physics bridge (330/LL7).

Row 50 (`H` hotkey Toggle HUD) is now considered WIRED — the LL1
`HUDOverlay` reads `shell._hud_visible` via the runtime overlay
layer, so the previously-flagged "flag flip only" concern from Top
10 fix #8 is resolved. Rows 315 (JJ4) and 317 (JJ7) share commit
`9b457e6` because the skeleton runtime and shadow builder shipped
together in a single sprint bundle. Row 328 (LL5) parity harness
touches every JJ+KK layer and is the recommended reference demo
for downstream engine users. Post-MM3 roll-up: **330 total,
313 WIRED (94.8%), 13 STUB (3.9%), 3 BROKEN (0.9%)**.
