...
- **Other call sites:** `PharosEngineExamples/examples/legacy/physics_projectile_demo.py:42` — legacy example, dies with the demo cleanup
...
- **Consumers in tests:**
  ...
  - `PharosEngineTests/tests/visual/test_vis_granular.py` — visual regression
- **Consumers in games:** none
- **Other call sites:** `PharosEngineExamples/examples/legacy/physics_sand_pile_demo.py:25` — legacy example
...
- **Consumers in games:**
  - **Ochema Circuit** — `systems/repair_system.py:8` (top-level), `PharosEngineTests/tests/test_repair_system.py` (5 in-method imports of `DeformRepairer`)
- **Strip status:** **blocked-on-Ochema-migration**
...
- **Consumers in tests:**
  - `PharosEngineTests/tests/test_pixel_struct.py:3` — top-level import
...
**Risks:**
- `PharosEngineTests/tests/visual/test_vis_granular.py` is a visual regression — verify
