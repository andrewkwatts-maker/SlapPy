...
`pharos-engine` PyPI wheel, explains what capability each extra
...
| `all`     | `pharos-engine[editor,assets,hud,math,video,audio,network]`         | Meta: everything except the heavy `ai` bundle    | ~150 MB    |
...
pip install pharos-engine[hud]

# Import glTF / OBJ meshes at editor / bake time.
pip install pharos-engine[assets]

# Full engine dev — editor + asset pipeline + HUD + math backend.
pip install pharos-engine[editor,assets,hud,math]

# Everything except the ~800 MB torch bundle.
pip install pharos-engine[all]

# Add AI features (Ollama client + Hugging Face pipelines).
pip install pharos-engine[all,ai]

# Contributing to the engine (tests + Rust wheel build).
pip install pharos-engine[dev]

# Multiplayer game.
pip install pharos-engine[hud,network,audio]
```
...
Which `pharos_engine.<subpackage>` requires which extra? All entries
...
| `pharos_engine.tools.video`                       | `video`        | `imageio-ffmpeg`, `opencv-python`        |
...
`lz4` deps + the whole `pharos_engine._core` Rust extension (~13 MB
total wheel weight). Roughly 90% of the engine surface still imports
cleanly:

* `pharos_engine.compute` — GPU compute pipelines (still needs a
  display for wgpu adapter, but `--headless` CI runners work with the
  llvmpipe / SwiftShader fallbacks).
* `pharos_engine.dynamics` — Rust-backed 2D physics.
* `pharos_engine.render` — null renderer path (`renderer.py` falls back
  to a no-op canvas when `wgpu.request_adapter()` returns `None`).
* `pharos_engine.scene`, `entity`, `components`, `event_bus`,
  `serialize` — all core ECS + YAML persistence.
* `pharos_engine.asset_import.stub_importer` — no-op stubs for FBX /
...
* `pharos_engine.testing` (golden-master API) — Pillow-only.

The following will raise `ImportDependencyError` until you install the
gating extra:

* Any mesh import via `pygltflib` → `pip install pharos-engine[assets]`.
* Any DearPyGui editor panel → `pip install pharos-engine[editor]`.
* Any Formula symbolic route → `pip install pharos-engine[math]`.
* Any audio playback → `pip install pharos-engine[audio]`.
...
pip install pharos-engine[dev,assets,audio,math]
```
...
install. This keeps `pip install pharos-engine` under 15 MB total.
...
* `python/pharos_engine/asset_import/gltf_importer.py` — glTF loader
  that soft-imports `pygltflib`.
* `python/pharos_engine/asset_import/stub_importer.py` — trimesh soft
  import for STL / PLY.
* `python/pharos_engine/audio.py`,
  `python/pharos_engine/audio_runtime.py` — sounddevice + soundfile
  soft imports.
* `python/pharos_engine/animation/video_import.py` — `av` soft import.
* `python/pharos_engine/ai/llm_client.py` — httpx soft import.
