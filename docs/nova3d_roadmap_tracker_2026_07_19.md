...
| Subsystem | Reason |
|---|---|
| Radiance-cascade GI (`engine/graphics/RadianceCascade.hpp`) | **Already ported** as WGSL rewrite; see `pharos_engine.gi` + `docs/gi_design.md`. |
| ReSTIR reservoir reuse | **Already ported**; part of `pharos_engine.gi`. |
...
| Procedural terrain (`engine/procedural/*`) | **Superseded** by existing `pharos_engine.iso` + heightmap layer path. |
