...
| **UU2** | `b29e601` | event_bus backcompat: restore module-level `global_bus` symbol pointing at `_DEFAULT_BUS` singleton (load-bearing for `pharos_editor.ui.debug_overlay`, `compute.library`, `compute.hull`, downstream games); widen `EventBus.unsubscribe(event_type, callback=None)` so legacy single-arg form drops every listener bound to the topic while modern two-arg form still removes a specific callback; module proxy mirrors same shape | 5 (`test_event_bus_backcompat.py`) |
