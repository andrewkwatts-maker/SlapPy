...
| 3.2 | Game-compat ≥80% of F1 (1178 combined) — **PASSING** (2026-07-08 +2) | Re-run YY3/ZZ3 harness against `H:/DaedalusSVN/Ochema Circuit/` + `H:/DaedalusSVN/Bullet Strata/` (**note**: repo dirs contain spaces — YY7 briefing path `OchemaCircuit/BulletStrata` was wrong; ZZ3 verified correct spaced paths) | Combined ≥ **942** passes (0.80 × 1178). **PASSING**: ZZ3 post-race re-run measured 1089 combined at HEAD `4e4c2dd` (Ochema 1039/70/17 after ZZ1 `7990501` Observable kwarg-swallow fix landed; Bullet Strata 50/4/0); F1 recovery **92.4%** (well above 80% YELLOW threshold, ~2.6 pp below 95% GREEN). Analysis: `docs/game_compat_2026_07_07.md` § 12 (YY3) + § 13 (ZZ3) + § 13.7 (post-ZZ1 addendum). Pre-tag operators may proceed to Step 1 with gate #12 status YELLOW / SHIP-AT-YELLOW per ship-decision doc § 9 Option F. |
