...
Every hardening round ships a `PharosEngineTests/tests/test_hardening_<module>.py` file
with negative-path coverage only — the positive path is already
exercised by the broader test suite. See
`PharosEngineTests/tests/test_hardening_eventbus.py` for the canonical shape.
...
- **Hand-authored** — opted in by the literal marker on line 1:
  ...
  `PharosEngineTests/tests/test_docs_api_template_conformance.py` asserts every
...
so `PharosEngineTests/tests/test_docs_links_resolve_all.py` resolves them — replace `LBLK`
...
`docs/sprint_5_doc_inventory.md`. `PharosEngineTests/tests/test_docs_inventory.py`
...
This is asserted by `PharosEngineTests/tests/test_dynamics_builder_conventions.py`. If you
...
- **No leading-underscore exports in `__all__`.** Public surface stays
  ...
  doc generator + `PharosEngineTests/tests/test_docs_engine_surface_complete.py`.
- **Alphabetise `__all__`** within each semantic group so diff churn
  stays minimal.
- **Negative-path hardening tests** live in
  `PharosEngineTests/tests/test_hardening_<module>.py` — one file per module, never share
...
`PharosEngineTests/tests/test_post_process_base.py`.

### Tests

Add a `PharosEngineTests/tests/test_<pass_name>_pass.py` smoke test exercising:
...
add a `PharosEngineTests/tests/test_hardening_postprocess.py` entry for the negative
path.
