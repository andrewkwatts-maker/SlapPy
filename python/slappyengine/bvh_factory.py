...
def build_bvh(primitives: Iterable[Any]) -> Optional[Any]:
    ...
    -------
    object or None
        ``None`` when :data:`slappyengine.config.MOBILE_DISABLE_BVH` is True
    ...
    from slappyengine import config as _cfg
...
def make_sdf_scene() -> Optional[Any]:
    ...
    -------
    SdfScene or None
        ``None`` when :data:`slappyengine.config.MOBILE_DISABLE_SDF` is True.
    ...
    try:
        from slappyengine._core import SdfScene  # type: ignore[attr-defined]
