...
:class:`slappyengine.ui.theme.user_themes.UserThemeStore` pattern for
post-process chain manifests: every shipping preset lives as a YAML
file under
``python/slappyengine/post_process/baked_chains/*.chain.yaml`` inside
the wheel, and on first use a baker copies each file into
``~/.slappyengine/postprocess_chains/`` so users can edit the YAML
...
* ``default``     bloom -> taa -> tonemap -> dither (matches
  :data:`slappyengine.post_process.DEFAULT_CHAIN`).
...
class ChainBaker:
    ...
    1. **Baked** (read-only):
       ``python/slappyengine/post_process/baked_chains/`` — shipped
       inside the wheel; the canonical source of every shipping preset.
    2. **User** (read/write):
       ``~/.slappyengine/postprocess_chains/`` — copied from baked on
    ...
    ----------
    user_dir:
        Override the default ``~/.slappyengine/postprocess_chains/``
    ...
    USER_DIR: Path = Path.home() / ".slappyengine" / "postprocess_chains"
