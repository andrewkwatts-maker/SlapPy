...
def record_or_smoke(
    ...
    * **Smoke branch**: emulates ``record`` without the rasterise / save
    ...
    ``stage`` must quack like :class:`slappyengine.studio.Stage` - it
    ...
    :func:`slappyengine.studio.record` loop steps).

    Args:
        stage: a :class:`slappyengine.studio.Stage` (or duck-equivalent).
