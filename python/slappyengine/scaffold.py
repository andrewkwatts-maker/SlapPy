...
subcommands defined in :mod:`slappyengine.cli` and can also be used as a
library::
...
* **Editor is optional** — if the ``slappyengine[editor]`` extra is not
...
* **Temp projects for lambdas** — :func:`create_temp_project` scaffolds a
  disposable project into ``~/.slappyengine/temp_projects/`` so users can
...
# Generated ${created_iso} by slappyengine ${engine_version}
# ---------------------------------------------------------------------------
...
def _make_app():
    ...
    Newer engine releases expose ``slappyengine.App``; older releases only
    expose :class:`slappyengine.Engine`.  We try App first and fall back to
    Engine so this scaffold works across versions.
    """
    try:
        from slappyengine import App  # type: ignore[attr-defined]
...
python -m pip install --upgrade slappy-engine pyyaml
...
python -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('slappyengine.ui.editor') else 1)"
if errorlevel 1 (
    echo.
    echo The SlapPyEngine editor is not installed.
    echo Install it with:  pip install "slappy-engine[editor]"
    echo.
    exit /b 1
)

python -m slappyengine.ui.editor "%PROJECT_DIR%" %*
set EC=%ERRORLEVEL%
endlocal & exit /b %EC%
"""
...
$probe = python -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('slappyengine.ui.editor') else 1)"
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "The SlapPyEngine editor is not installed."
    Write-Host 'Install it with:  pip install "slappy-engine[editor]"'
    Write-Host ""
    exit 1
}

python -m slappyengine.ui.editor $ProjectDir @args
exit $LASTEXITCODE
"""
...
if ! python -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('slappyengine.ui.editor') else 1)"; then
    echo
    echo "The SlapPyEngine editor is not installed."
    echo 'Install it with:  pip install "slappy-engine[editor]"'
    echo
...
def _engine_version() -> str:
    try:
        from slappyengine import __version__ as v  # type: ignore
        return str(v)
    except Exception:
        return "unknown"
...
def editor_installed() -> bool:
    """Return True if the ``slappyengine[editor]`` extra can be imported."""
    import importlib.util
    return importlib.util.find_spec("slappyengine.ui.editor") is not None


def temp_projects_root() -> Path:
    """Directory that holds temp projects created by :func:`create_temp_project`."""
    root = Path(os.environ.get("SLAPPY_TEMP_ROOT", str(Path.home() / ".slappyengine" / "temp_projects")))
...
def create_temp_project(
    *,
    ...
    """Create a disposable project in ``~/.slappyengine/temp_projects/``.
...
def launch_project(
    ...
    *,
    ...
    editor
        When True, invoke ``python -m slappyengine.ui.editor <path>`` instead
    ...
    if editor:
        if not editor_installed():
            raise RuntimeError(
                "editor extra not installed — pip install 'slappy-engine[editor]'"
            )
        cmd = [sys.executable, "-m", "slappyengine.ui.editor", str(p)]
    else:
