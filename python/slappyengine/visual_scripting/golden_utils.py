...
(see :func:`slappyengine.visual_scripting.node._gen_id`). To make the
...
# repo layout: ``python/slappyengine/visual_scripting/golden_utils.py``
...
def canonical_graph_yaml(graph: NodeGraph) -> str:
    ...
    :func:`slappyengine.visual_scripting.node._gen_id`) so two runs of
