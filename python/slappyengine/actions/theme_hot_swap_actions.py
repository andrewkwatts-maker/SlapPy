...
Backs the ``theme.hot_swap`` :class:`~slappyengine.tool_router.ToolAction`
...
it against :func:`slappyengine.ui.theme.get_theme` before applying via
:func:`~slappyengine.ui.theme.apply_theme`.
...
the ``slappyengine.ui.theme`` package directly.
...
def _import_theme_module() -> Any:
    try:
        import slappyengine.ui.theme as theme_mod
...
def hot_swap(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if theme_mod is None:
        return {
            "status": "error",
            "message": "slappyengine.ui.theme failed to import",
        }
