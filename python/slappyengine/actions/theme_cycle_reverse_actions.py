...
Backs the ``theme.cycle_reverse`` :class:`~slappyengine.tool_router.ToolAction`
row added by the DD1 STUB-triage sprint tick (round 7 after
X3 / Y1 / Z7 / AA1 / BB1 / CC1).

Symmetric to
:func:`slappyengine.actions.theme_actions.cycle_theme`: same shell-hook
preference, same headless fallback over
:func:`slappyengine.ui.theme.list_registered_themes`, but the cursor
advances in the *opposite* direction. The two actions share
:data:`slappyengine.actions.theme_actions._THEME_CURSOR` so a forward
...
def cycle_theme_reverse(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    3. Headless fallback — rewind the shared cursor and call
       :func:`~slappyengine.ui.theme.apply_theme` on the previous entry.
