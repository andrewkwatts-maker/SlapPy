...
Backs the ``view.reset_layout`` :class:`~slappyengine.tool_router.ToolAction`
row. Restores the DEFAULT preset via
:func:`~slappyengine.ui.editor.layout_presets.apply_preset` when a shell
...
def reset_layout(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    * When ``ctx["shell"]`` exposes ``apply_layout_preset(name)``, that
      ...
      Ctrl+0 menu binding).
    * Otherwise :func:`slappyengine.ui.editor.layout_presets.apply_preset`
