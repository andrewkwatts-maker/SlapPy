...
Every helper here is a thin façade over :class:`slappyengine.App` (for
spawning / mutating handles) or over :class:`slappyengine.scene.Scene`
...
# left to :mod:`slappyengine.shader_gen`; the helper returns a bare
...
def load_model(path: str | Path, *, app: "App | None" = None) -> "ModelHandle":
    """Load a model asset — delegates to :func:`slappyengine.load_model`."""
...
def load_shader(path: str | Path, *, app: "App | None" = None) -> ShaderHandle:
    ...
    or hand it off to :mod:`slappyengine.shader_gen`.
