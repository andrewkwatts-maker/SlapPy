...
* Golden-master on first run. If no baseline exists for a scene name
  the rendered PNG is written into ``slappyengine/testing/baselines/``
...
def _resolve_diff_dir() -> Path:
    # python/slappyengine/testing/__init__.py → repo_root/docs/visual_diffs
    here = Path(__file__).resolve()
    repo_root = here.parents[3]  # …/repo/python/slappyengine/testing → …/repo
