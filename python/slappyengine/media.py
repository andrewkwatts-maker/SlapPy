...
* :func:`slappyengine.tools.video.write_gif` for ``.gif``,
* :func:`slappyengine.tools.video.write_mp4` for ``.mp4``,
* :func:`slappyengine.tools.video.have_mp4_support` for detection.

For *streaming* (frame-by-frame) capture use
:class:`slappyengine.physics.video.VideoWriter` directly. ``save_frames``
...
def _load_media_defaults() -> dict:
    ...
    try:
        ...
        return defaults
    # Walk up from this file: python/slappyengine/media.py -> repo root -> config/
...
def have_ffmpeg() -> bool:
    ...
    Mirrors :func:`slappyengine.tools.video.have_mp4_support` but is the
    canonical entry point for ``media``-level callers.
    """
    from slappyengine.tools.video import have_mp4_support
...
def save_frames(
    ...
    *,
    ...
    if wants_mp4:
        from slappyengine.tools.video import write_mp4
        return write_mp4(frames, dest, fps=fps, quality=quality)

    from slappyengine.tools.video import write_gif
    return write_gif(
        frames,
        dest,
        fps=fps,
