"""``slappyengine.prefabs`` — reusable entity templates via ``.prefab.yaml``.
...
* A body-spec dict — the shape passed to a
  :class:`slappyengine.dynamics.World` builder (one of the seven
...
``~/.slappyengine/prefabs/`` on first use so downstream code can edit
