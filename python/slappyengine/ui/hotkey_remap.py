...
The :mod:`slappyengine.ui.user_overrides` layer (X6) already loads user
YAML hotkey files from ``~/.slappyengine/ui/hotkeys/`` and folds them
into :class:`~slappyengine.ui.editor.notebook_hotkeys.NotebookHotkeys`.
...
:data:`slappyengine.tool_router.REGISTRY`, a first-launch
:func:`bake_defaults` bootstrapper, and a :class:`HotkeyRemapWatcher`
that subclasses the X6 :class:`~slappyengine.ui.user_overrides.WatcherHandle`
...
::
    ...
    python/slappyengine/ui/hotkeys/baked/
...
* Hard boundary: this module never imports from
  :mod:`slappyengine.ui.user_overrides` at module scope. The subclassed
...
class HotkeyBinding:
    ...
    action_id:
        The :class:`~slappyengine.tool_router.ToolAction` identifier the
    ...
    def validate(self, router: Any) -> list[str]:
        ...
        if not callable(has_action):
            raise TypeError(
                ...
                "did you forget to pass slappyengine.tool_router.REGISTRY?"
            )
        seen: set[str] = set()
        unknown: list[str] = []
        for b in self._bindings.values():
...
USER_HOTKEY_DIR: Path = Path.home() / ".slappyengine" / "ui" / "hotkeys"
...
def default_hotkey_map() -> HotkeyMap:
    ...
    Imports :mod:`slappyengine.ui.editor.notebook_hotkeys` lazily so this
...
class HotkeyRemapWatcher:
    ...
    Wraps :class:`slappyengine.ui.user_overrides.UserOverrideLoader.watch_dir`
