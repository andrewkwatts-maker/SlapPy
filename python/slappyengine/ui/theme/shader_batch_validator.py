...
:func:`~slappyengine.ui.theme.shader_lint.lint_wgsl` linter (which
...
def _repo_root() -> Path:
    ...
    # ui/theme/ -> ui/ -> slappyengine/ -> python/ -> repo root.
    for parent in here.parents:
        if (parent / "python" / "slappyengine").is_dir():
            return parent
...
def _package_root() -> Path:
    """Locate the ``python/slappyengine`` directory on disk."""
    return _repo_root() / "python" / "slappyengine"


#: Directories whose ``*.wgsl`` files get validated by the sweep.
#: Each entry is ``(library_name, relative_path)``. Paths are resolved
#: relative to the ``python/slappyengine`` package root except where
#: noted.
...
def save_shader_manifest(out_path: str | Path) -> Path:
    ...
    payload = {
        "schema": "slappyengine.shader_manifest",
        "version": 1,
