...
baked into the wheel (``python/slappyengine/ui/theme/themes/_baked/``).
On first launch a :class:`UserThemeStore` copies each baked file into
``~/.slappyengine/themes/`` so users can open, edit, and save the YAML
...
:meth:`slappyengine.ui.theme.ThemeSpec.to_yaml`. Round-trip is
...
class UserThemeStore:
    """Manages user-editable theme files under ``~/.slappyengine/themes/``.

    Themes live in two locations:

    1. **Baked** (read-only):
       ``python/slappyengine/ui/theme/themes/_baked/`` — shipped inside
       the wheel; the canonical source of every built-in theme.
    2. **User** (read/write):
       ``~/.slappyengine/themes/`` — copied from baked on first launch.
    ...
    ----------
    user_dir:
        Override the default ``~/.slappyengine/themes/`` location — used
    ...
    USER_DIR: Path = Path.home() / ".slappyengine" / "themes"
...
def bake_default_themes(baked_dir: Path | str | None = None) -> list[Path]:
    ...
    list[pathlib.Path]
        ...
        diary-family rollout in :mod:`slappyengine.ui.theme.themes`).
    """
