...
(:mod:`slappyengine.ui.theme.shader_lint`) with a filesystem watcher
that re-runs :func:`~slappyengine.ui.theme.shader_lint.lint_wgsl`
whenever a WGSL file changes on disk.

The intended use cases are:

* **Live iteration on user-override shaders** — creators drop
  ``.wgsl`` files under ``~/.slappyengine/ui/shaders/<library>/`` and
...
pattern used by :mod:`slappyengine.ui.user_overrides`:
...
def _default_user_shader_dirs(user_dir: Path | None) -> list[Path]:
    ...
    if user_dir is None:
        user_dir = Path.home() / ".slappyengine" / "ui" / "shaders"
    else:
...
def start_default_hot_reload(
    ...
    *,
    ...
    user_dir:
        ...
        default (``~/.slappyengine/ui/shaders/``); pass a concrete
