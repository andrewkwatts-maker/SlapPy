...
``slappyengine.compute``'s GPU compute pipeline rather than only
...
def compile_wgsl_background(spec: WGSLShaderSpec) -> np.ndarray:
    ...
    :class:`~slappyengine.compute.ComputePipeline` and the resulting
...
def _dispatch_wgpu(spec: WGSLShaderSpec) -> np.ndarray:  # pragma: no cover
    """GPU path — real dispatch through ``slappyengine.compute``.

    Only reached when ``wgpu`` imported. The path is deliberately
    conservative: it *tries* to run through
    :class:`~slappyengine.compute.ComputePipeline` and, if no live
    :class:`~slappyengine.gpu.context.GPUContext` is registered, falls
