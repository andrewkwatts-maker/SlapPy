...
``~/.slappyengine/default_layout.yaml`` so the user's preferred chrome
...
class LayoutPersistence:
    ...
    project_root:
        ...
        ``~/.slappyengine/default_layout.yaml`` file so the editor still
    ...
    def get_file_path(self) -> Path:
        ...
        ``~/.slappyengine/default_layout.yaml``.
        """
        if self._project_root is not None:
            return (
                self._project_root / self.LAYOUT_DIR / self.LAYOUT_FILE
            )
        return Path.home() / ".slappyengine" / self.FALLBACK_FILE
    ...
    def save(self, layout: EditorLayout) -> None:
        ...
        Creates the ``.slappy/`` (or ``~/.slappyengine/``) directory if
