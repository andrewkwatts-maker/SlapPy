...
3. :mod:`slappyengine.telemetry` events via :meth:`subscribe_to_telemetry`.
...
class LogMessage:
    ...
    source:
        Dotted module name (e.g. ``"slappyengine.dynamics"``).
    message:
...
class NotebookMessageLog:
    ...
    shell using :class:`~slappyengine.ui.editor.movable_panel.MovablePanelWindow`).
    ...
    def subscribe_to_telemetry(self, pattern: str = "*") -> int:
        """Subscribe to :mod:`slappyengine.telemetry` events.
