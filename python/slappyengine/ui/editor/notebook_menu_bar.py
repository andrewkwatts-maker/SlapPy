...
Reads the :data:`slappyengine.tool_router.REGISTRY` and generates a diary-
...
an optional :class:`~slappyengine.ui.hotkey_remap.HotkeyMap` so users see
...
* Data sources:
  - :class:`slappyengine.tool_router.ToolRouter` — action list + labels.
  - :class:`slappyengine.ui.hotkey_remap.HotkeyMap` — combo -> action_id.
...
def mark_dpg_context_live(is_live: bool) -> None:
    ...
    :class:`slappyengine.ui.editor.shell.EditorShell` calls this after
...
class NotebookMenuBar:
    ...
    ----------
    router:
        Optional :class:`~slappyengine.tool_router.ToolRouter`. Set via
        constructor or :meth:`set_router`. When ``None`` the panel
        renders an empty menu structure until the router is bound.
    hotkey_map:
        Optional :class:`~slappyengine.ui.hotkey_remap.HotkeyMap`. When
