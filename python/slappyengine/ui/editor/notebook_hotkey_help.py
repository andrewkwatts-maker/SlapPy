...
:data:`slappyengine.tool_router.REGISTRY`; the right column hosts a
...
# in :mod:`slappyengine.ui.editor.notebook_prefab_menu`.
...
#: :data:`slappyengine.tool_router.REGISTRY`.
...
class NotebookHotkeyHelp:
    ...
    router:
        Optional :class:`~slappyengine.tool_router.ToolRouter` used to
        resolve ``action_id`` -> ``(label, category)``. When ``None``
        the panel imports :data:`slappyengine.tool_router.REGISTRY`
    ...
    def _resolve_router(self) -> Any | None:
        ...
        if self._router is not None:
            return self._router
        try:
            from slappyengine.tool_router import REGISTRY  # type: ignore
        except Exception:
            return None
