...
:class:`slappyengine.studio.Stage`) and the RIGHT page is the script
...
* ``python/slappyengine/ui/editor/notebook_code_panel.py`` — sibling
  diary-styled panel; the soft-DPG / call-log / theme-listener
  contracts are mirrored here.
* ``python/slappyengine/ui/editor/notebook_inspector.py`` — field-journal
  styling reference (washi-tape sections, sticker glyphs).

Headless / soft-import contract
-------------------------------

* :class:`slappyengine.studio.Stage` is imported lazily inside
...
_STUDIO_MISSING_HINT = (
    "slappyengine.studio.Stage unavailable - viewport idle."
)
...
def _try_import_studio() -> Any | None:
    """Return ``slappyengine.studio`` or ``None`` on import error.
    ...
    """
    try:
        from slappyengine import studio  # noqa: WPS433
