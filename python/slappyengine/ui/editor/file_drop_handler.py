...
* ``*.wgsl`` / ``*.glsl`` → copy to ``~/.slappyengine/ui/shaders/``.
...
* Companion of :mod:`slappyengine.ui.editor.notebook_prefab_menu` —
...
* Style / patterns: dataclass event + enum action + registered
  callback (mirrors :mod:`slappyengine.ui.editor.notebook_command_palette`).
"""
...
class FileDropHandler:
    ...
    def handle_drop(
        ...
        ctx:
            ...
            :class:`slappyengine.ui.editor.shell.EditorShell` instance
...
DEFAULT_SHADER_DIR: Path = Path.home() / ".slappyengine" / "ui" / "shaders"
...
def _handle_prefab_spawn(path: Path, event: FileDropEvent, ctx: Any) -> None:
    ...
    from slappyengine.prefabs import Prefab  # local import — avoids cycle
...
def default_handlers() -> dict[DropAction, HandlerCallback]:
    ...
    * ``prefab_library`` — :class:`slappyengine.prefabs.PrefabLibrary`.
    * ``world``          — target :class:`slappyengine.dynamics.World`.
    * ``user_theme_store`` — :class:`slappyengine.ui.theme.user_themes.UserThemeStore`.
