...
* The AI plumbing (``slappyengine.ai.ollama_manager`` /
  ``slappyengine.ai.code_sync``) is imported lazily inside the action
...
def _try_import_code_sync() -> tuple[Any, Any, Any] | None:
    ...
    Imported lazily so the editor still loads when ``slappyengine[ai]``
    isn't installed.
    """
    try:
        from slappyengine.ai.code_sync import (
            code_to_prompt,
            prompt_path_for,
            prompt_to_code,
        )
...
def _try_make_llm_client() -> Any | None:
    ...
    """
    try:
        from slappyengine.ai.ollama_manager import (
            OllamaManager,
            load_ai_settings,
        )
    except Exception:
...
class NotebookCodePanel:
    ...
    def _add_sticker_corner(self) -> None:
        ...
        try:
            from slappyengine.ui.widgets.sticker_corner import add_sticker_corner
