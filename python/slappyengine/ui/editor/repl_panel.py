...
:class:`slappyengine.App` from a live Python prompt: type a snippet,
...
* **Discoverable.** The default namespace pre-imports
  :mod:`slappyengine`, the helpers module, and pins the current
...
``slappyengine.ui.editor``. The shell instantiates the panel, hands it a
...
from slappyengine.editor import helpers as _helpers
...
class REPLPanel:
    ...
    ----------
    app
        The bound :class:`slappyengine.App` — populated at construction
        or on first submit.
    scene
    ...
    def _build_namespace(self) -> dict[str, Any]:
        ...
        * The full :mod:`slappyengine` module + every symbol in its
        ...
        * Anything the caller passed via ``extra_namespace=``.
        ...
        if self.app is None:
            try:
                self.app = slappyengine.App._get_implicit()
            except Exception:
                self.app = None
        ...
        for name in getattr(slappyengine, "__all__", []):
            if name in ns:
                continue
            try:
                ns[name] = getattr(slappyengine, name)
...
def make_repl_panel(app: Any = None, scene: Any = None) -> REPLPanel:
    ...
    Used by :class:`slappyengine.ui.editor.shell.EditorShell` so the
