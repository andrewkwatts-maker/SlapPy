...
from slappyengine._compat import (
    CellMaterial,
    cell_material_for,
)
...
from slappyengine.physics.broadphase import SpatialHashBroadphase
...
class FrontierYamlConfig:
    ...
    :class:`slappyengine.physics.frontier.FrontierConfig`. This wrapper
...
class PhysicsWorld:
    ...
    def __init__(
        ...
        # ``slappyengine.physics.memory_budget._install_memory_section_on_physics_yaml``
        # at import time, so it's always present here.  Import lazily to
        # avoid a circular import at module load.
        from slappyengine.physics.memory_budget import (
        ...
        # ``slappyengine.physics.broadphase`` for the reasoning.
    ...
    def _try_spawn_fragments(self) -> None:
        ...
        from slappyengine.physics.body import PhysicsBody as _PB
    ...
    def _pressure_project_arrays(
        ...
        * The residual is restricted to fluid cells each sweep so cells
        ...
        if bool(getattr(mat, "use_multigrid", False)):
            from slappyengine.physics.pressure_multigrid import vcycle_project_v
