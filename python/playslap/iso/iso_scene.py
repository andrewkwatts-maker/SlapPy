...
class IsoScene:
    ...
    ``IsoScene`` owns an :class:`~playslap.iso.iso_grid.IsoGrid`,
    an :class:`~playslap.iso.iso_camera.IsoCamera`, and a list of
    :class:`~playslap.iso.iso_entity.IsoEntity` objects.  It can be used
    ...
    def sorted_render_list(
        ...
        - ``"data"``: The :class:`~playslap.iso.iso_grid.IsoCell` (tiles)
          or :class:`~playslap.iso.iso_entity.IsoEntity` (entities).
        - ``"dk"`` *(entities only)*: The raw depth key float.

        Tiles already carry an implicit depth key from
        :meth:`~playslap.iso.iso_grid.IsoGrid.sorted_cells`; entities are
