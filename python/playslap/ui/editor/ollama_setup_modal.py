...
1. :class:`AiOptInDialog` — shown on first launch; asks whether the user
   ...
   ``~/.playslap/ai_settings.json`` so it is never asked again.
...
class AiOptInDialog:
    ...
    ``~/.playslap/ai_settings.json``.  Returns the chosen model tag
    ...
    def _on_enable(self, *_) -> None:
        ...
        from playslap.ai.ollama_manager import save_ai_settings
...
class OllamaSetupModal:
    ...
    def show(self, model: str | None = None) -> bool:
        import dearpygui.dearpygui as dpg
        from playslap.ai.ollama_manager import OllamaManager
