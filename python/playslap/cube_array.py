...
from playslap.animation.graph import AnimationGraph
...
class CubeArray(RenderTarget):
    ...
    def from_video(cls, path: str, name: str = "", use_as: str = "frames",
        ...
        import numpy as np
        try:
            from playslap.animation.video_import import extract_frames
            frame_arrays = extract_frames(path, max_frames=max_frames)
        except ImportError as e:
            raise ImportError(f"Video import requires pip install playslap[video]: {e}") from e
        inst = cls(name=name)
