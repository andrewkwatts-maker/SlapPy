...
class TestTile:
    def test_tile_coord_stored(self):
        from slappyengine.landscape import TileCoord, Tile
        coord = TileCoord(2, 3)
...
class TestLandscape:
    ...
    def test_tile_paths(self):
        from slappyengine.landscape import Landscape, TileCoord
        with tempfile.TemporaryDirectory() as tmpdir:
            lnd = Landscape(tile_size=64, tile_dir=tmpdir)
            coord = TileCoord(5, 3)
...
class TestKnownNodeTypes:
    ...
    def test_known_port_types_consistent(self):
        from slappyengine.material.graph_schema import KNOWN_NODE_TYPES, KNOWN_PORT_TYPES
