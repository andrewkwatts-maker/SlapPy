...
Backs the ``view.reset_layout`` :class:`~pharos_editor.tool_router.ToolAction`
row. Restores the DEFAULT preset via
:func:`~pharos_editor.ui.editor.layout_presets.apply_preset` when a shell
...
def reset_layout(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    * When ``ctx["shell"]`` exposes ``apply_layout_preset(name)``, that
      ...
      Ctrl+0 menu binding).
    * Otherwise :func:`pharos_editor.ui.editor.layout_presets.apply_preset`
