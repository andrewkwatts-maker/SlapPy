...
def hud_command_to_text(cmd: DrawCommand, atlas: Any) -> Any:
    ...
    atlas:
        A :class:`~pharos_engine.text.SDFGlyphAtlas` supplying glyph
        metrics. When ``None`` the function returns ``None`` so callers
        in headless CI can skip text without special-casing.

    Returns
    -------
    A :class:`~pharos_engine.text.TextMesh` — or ``None`` when the atlas
    is missing.
    """
    if cmd.kind != "text":
        raise ValueError(
    ...
    from pharos_engine.text import SDFTextRenderer
...
class HUDOverlay:
    ...
    renderer:
        ...
        The real :class:`~pharos_engine.render.Renderer` fits; so does a
        stub with the same signature for tests.
    camera_2d:
        A :class:`~pharos_engine.render.camera.Camera2D` used to derive
