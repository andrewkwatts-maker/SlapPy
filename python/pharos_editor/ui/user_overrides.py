...
* A missing ``~/.pharos_engine/ui/`` tree silently produces an empty
...
#: Directory names inside ``~/.pharos_engine/ui/`` — the constant is the
...
class UserOverrideLoader:
    """Discovers + loads user overrides from ``~/.pharos_engine/ui/``.
    ...
    root:
        ...
        ``Path.home() / ".pharos_engine" / "ui"``.  Tests pass a tmp path.
    """

    ROOT: Path = Path.home() / ".pharos_engine" / "ui"
    ...
    def ensure_scaffolded(self) -> None:
        ...
        untouched. On the first launch (empty ``~/.pharos_engine/ui``)
