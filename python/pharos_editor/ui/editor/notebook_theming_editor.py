...
def _soft_import_page_linings() -> list[str]:
    ...
    try:
        from pharos_editor.ui.theme.page_linings import (  # type: ignore
            list_linings,
        )
        result = list_linings()
...
def _soft_import_edge_strokes() -> list[str]:
    ...
    try:
        from pharos_editor.ui.theme.edge_strokes import (  # type: ignore
            list_strokes,
        )
        result = list_strokes()
        if isinstance(result, (list, tuple)) and result:
...
def _soft_import_creatures() -> list[str]:
    ...
    try:
        from pharos_editor.ui.theme.creatures import builtin as _b  # type: ignore
...
def _soft_import_user_store() -> Any | None:
    ...
    """
    try:
        from pharos_editor.ui.theme.user_theme_store import (  # type: ignore
            UserThemeStore,
        )
...
class NotebookThemingEditor:
    ...
    def _mutate_spec_palette(
        ...
        if spec is None:
            return
        try:
            from pharos_editor.ui.theme.theme_spec import Color as _Color
        except Exception:
