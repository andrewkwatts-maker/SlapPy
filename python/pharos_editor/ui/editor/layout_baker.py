...
Mirrors the :class:`pharos_engine.post_process.chain_baker.ChainBaker`
pattern for editor layout snapshots: every shipping preset lives as a
YAML file under
``python/pharos_engine/ui/editor/baked_layouts/*.layout.yaml`` inside
the wheel, and on first use a baker copies each file into
``~/.pharos_engine/layouts/`` so users can edit the YAML freely without
...
* ``default``       standard four-pane notebook (toolbar / outliner /
  ...
  :data:`pharos_editor.ui.editor.default_layouts.DEFAULT_LAYOUT`.
...
class LayoutBaker:
    ...
    1. **Baked** (read-only):
       ``python/pharos_engine/ui/editor/baked_layouts/`` — shipped
       inside the wheel; the canonical source of every shipping preset.
    2. **User** (read/write):
       ``~/.pharos_engine/layouts/`` — copied from baked on first
    ...
    ----------
    user_dir:
        Override the default ``~/.pharos_engine/layouts/`` location —
    ...
    USER_DIR: Path = Path.home() / ".pharos_engine" / "layouts"
