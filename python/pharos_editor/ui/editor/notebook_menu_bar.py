...
Reads the :data:`pharos_editor.tool_router.REGISTRY` and generates a diary-
...
an optional :class:`~pharos_editor.ui.hotkey_remap.HotkeyMap` so users see
...
* Data sources:
  - :class:`pharos_editor.tool_router.ToolRouter` — action list + labels.
  - :class:`pharos_editor.ui.hotkey_remap.HotkeyMap` — combo -> action_id.
...
def mark_dpg_context_live(is_live: bool) -> None:
    ...
    :class:`pharos_editor.ui.editor.shell.EditorShell` calls this after
...
class NotebookMenuBar:
    ...
    ----------
    router:
        Optional :class:`~pharos_editor.tool_router.ToolRouter`. Set via
        constructor or :meth:`set_router`. When ``None`` the panel
        renders an empty menu structure until the router is bound.
    hotkey_map:
        Optional :class:`~pharos_editor.ui.hotkey_remap.HotkeyMap`. When
