...
:data:`pharos_editor.tool_router.REGISTRY`; the right column hosts a
...
* Data source: :class:`pharos_editor.ui.hotkey_remap.HotkeyMap` (AA7).
* Preset dropdown loads baked YAMLs from
  :data:`pharos_editor.ui.hotkey_remap.BAKED_HOTKEY_DIR` — the same
...
* Action metadata (label + category) is resolved through
  ...
  :data:`pharos_editor.tool_router.REGISTRY` — unknown ``action_id``
...
# in :mod:`pharos_editor.ui.editor.notebook_prefab_menu`.
...
def mark_dpg_context_live(is_live: bool) -> None:
    ...
    Called by :class:`pharos_editor.ui.editor.shell.EditorShell` after
...
#: :data:`pharos_editor.tool_router.REGISTRY`.
...
class NotebookHotkeyHelp:
    ...
    router:
        Optional :class:`~pharos_editor.tool_router.ToolRouter` used to
        resolve ``action_id`` -> ``(label, category)``. When ``None``
        the panel imports :data:`pharos_editor.tool_router.REGISTRY`
    ...
    def _resolve_router(self) -> Any | None:
        ...
        if self._router is not None:
            return self._router
        try:
            from pharos_editor.tool_router import REGISTRY  # type: ignore
        except Exception:
            return None
