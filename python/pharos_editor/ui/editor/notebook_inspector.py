...
:class:`pharos_editor.ui.editor.property_inspector.PropertyInspector`. It
...
but wraps each section in a :class:`~pharos_editor.ui.widgets.WashiPanel`
"field journal" page, swaps drag-floats for
:class:`~pharos_editor.ui.widgets.HighlighterSlider` strips, swaps bool
checkboxes for :class:`~pharos_editor.ui.widgets.HeartCheckbox` hearts,
and inserts a :class:`~pharos_editor.ui.widgets.DoodleSeparator` between
sections.
...
from pharos_editor.ui.editor.notebook_inspector_dispatch import (
...
def _resolve_engine_version() -> str:
    ...
    """
    try:
        from pharos_engine import __version__ as _v
        return str(_v)
    except Exception:
        return "?"
...
def _resolve_project_banner() -> tuple[str, str] | None:
    ...
    :mod:`pharos_engine.projects` and finally returns ``None`` so the
    ...
    try:
        from pharos_engine.projects import get_active_project  # type: ignore[attr-defined]
...
def _ensure_recent_activity_buffer() -> list[str]:
    ...
    try:
        from pharos_engine.event_bus import global_bus  # type: ignore[attr-defined]
...
class NotebookInspector(InspectorDispatchMixin):
    ...
    :mod:`pharos_editor.ui.editor.property_inspector` so dataclass fields
    work for free.
    ...
    def _render_section(
        ...
        *,
        ...
        from pharos_editor.ui.widgets import WashiPanel
    ...
    def _add_doodle_separator(self, dpg: Any, style: str) -> None:
        try:
            from pharos_editor.ui.widgets import DoodleSeparator
