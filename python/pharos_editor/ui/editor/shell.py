...
class EditorShell:
    ...
    def __init__(
        ...
        # is missing.
        try:
            from pharos_editor.ui.editor.snap_manager import SnapManager
            self._snap_manager = SnapManager()
            self._snap_manager.set_viewport_size(self._width, self._height)
        except Exception:
            self._snap_manager = None
        try:
            from pharos_editor.ui.editor.dock_zones import DockZoneManager
        ...
        # user-wide ``~/.pharos_engine/default_layout.yaml`` when no project
        ...
        from pharos_editor.ui.editor.layout_persistence import LayoutPersistence
        ...
        try:
            from pharos_editor.ui.editor.diary_shell import DiaryShell
    ...
    def _dispatch_editor_command(self, command: str) -> None:
        ...
        :data:`pharos_editor.tool_router.REGISTRY`. This dispatcher
        ...
        """
        from pharos_editor.tool_router import REGISTRY as _ROUTER
    ...
    def toggle_panel(self, panel_id: str) -> bool:
        ...
        """
        from pharos_editor.ui.editor.layout_presets import PanelLayoutState
    ...
    def register_panel(self, panel) -> None:
        ...
        * The panel is routed to a *named slot* on the shell when its
        ...
        # — keeping :mod:`pharos_editor.ui.editor.shell` import-cheap.
    ...
    def setup_theme_subsystem(self) -> None:
        ...
        from pharos_editor.ui.theme.creatures import CreatureBusAdapter
        ...
        try:
            from pharos_editor.ui.theme import get_baked_background
    ...
    # ------------------------------------------------------------------
    # User overrides — ~/.pharos_engine/ui/
    # ------------------------------------------------------------------

    def load_user_overrides(self) -> None:
        """Discover + fold user overrides from ``~/.pharos_engine/ui/``.
        ...
        * WGSL shaders are handed to the matching theme registry.
        ...
        if bundle.spawn_actions:
            try:
                from pharos_editor.ui.editor import spawn_menu as _sm
        ...
        for shader_id, wgsl in bundle.shaders.items():
            ...
            try:
                if kind == "page_linings":
                    ...
                    from pharos_editor.ui.theme.washi_tape.library import (
                    ...
                    from pharos_editor.ui.theme.edge_strokes.library import (
    ...
    def setup_notebook_panels(self) -> None:
        ...
        if self._post_process_panel is None:
            try:
                from pharos_editor.ui.editor.notebook_post_process_panel import (
        ...
        if self._viewport_3d_panel is None:
            try:
                from pharos_editor.ui.editor.viewport_3d_panel import (
                    Viewport3DPanel,
                )
    ...
    def setup(self) -> None:  # noqa: C901 (complexity — UI layout)
        ...
        # outliner is built.
        from pharos_editor.ui.editor import spawn_menu as _spawn_menu
        self._spawn_menu = _spawn_menu

        # ── User-override layer — ~/.pharos_engine/ui/ ─────────────────────
        ...
        if getattr(self, "_repl_panel", None) is None:
            try:
                ...
                _app_ref = None
                try:
                    from pharos_engine.app import App as _AppCls
        ...
        if getattr(self, "_wgsl_panel", None) is None:
            try:
                from pharos_editor.ui.editor.wgsl_editor_panel import (
                    WGSLEditorPanel,
                )
        ...
        if self._code_mode_panel is None:
            try:
                from pharos_editor.ui.editor.code_mode_panel import (
                    CodeModePanel,
                )
    ...
    def run(self) -> None:
        ...
        try:
            from pharos_editor.ui.theme.dpg_bridge import mark_dpg_context_ready
    ...
    def set_project(self, project: "Project") -> None:
        ...
        from pharos_engine.projects import Project as _Project
    ...
    def get_diary_page(self) -> "NotebookDiaryPage":
        ...
        left (running a small :class:`pharos_engine.studio.Stage`) and the
    ...
    def _populate_layout_presets_menu(self) -> None:
        ...
        try:
            ...
            return
        from pharos_editor.ui.editor.layout_presets import PRESETS
    ...
    def menu_about(self) -> dict:
        ...
        """
        try:
            from pharos_engine import __version__ as _ver
        except Exception:
            _ver = "0.0.0"
        info = {
            "version": _ver,
