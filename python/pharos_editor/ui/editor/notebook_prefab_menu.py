...
An extension sibling of :class:`pharos_editor.ui.editor.notebook_spawn_menu.NotebookSpawnMenu`.
...
recipes registered in a :class:`pharos_engine.prefabs.PrefabLibrary` that
...
the ``pharos_engine.prefabs`` package (Y3 landing, read-only). It
...
* Style reference: :mod:`pharos_editor.ui.editor.notebook_spawn_menu`
  (``SpawnCard`` dataclass + ``_build_cards`` / hover / recents layout).
* Library reference: :class:`pharos_engine.prefabs.library.PrefabLibrary`.
"""
...
class NotebookPrefabMenu:
    ...
    library:
        ...
        + overlays ``~/.pharos_engine/prefabs/`` so the panel has content
        out of the box.
    on_spawn:
    ...
    def _render_empty_state(
        ...
        if reason == "empty":
            ...
            hint = "Add *.prefab.yaml files to ~/.pharos_engine/prefabs/"
        else:
    ...
    def _dpg_context_alive(dpg: Any) -> bool:
        ...
        :data:`pharos_editor.ui.editor.notebook_prefab_menu.
