...
def default_snapshot_dir(project_name: str) -> Path:
    """Return ``~/.pharos_engine/autosave/<project_name>/`` for *project_name*.
    ...
    return Path.home() / ".pharos_engine" / "autosave" / safe
...
def _encode_payload(payload: Any) -> Any:
    ...
    * anything else → ``repr(payload)`` as a str fallback.
    ...
    if isinstance(payload, (bytes, bytearray)):
        return {
            "__pharos_engine_autosave_bytes__": True,
...
def _decode_payload(payload: Any) -> Any:
    ...
    if isinstance(payload, dict):
        if payload.get("__pharos_engine_autosave_bytes__") is True and "b64" in payload:
            try:
...
class AutosaveState:
    ...
    snapshot_dir:
        ...
        ``~/.pharos_engine/autosave/<project_name>/`` via
    ...
    snapshot_dir: Path = field(default_factory=lambda: Path.home() / ".pharos_engine" / "autosave")
...
class AutosaveManager:
    ...
    project:
        ...
        :class:`pharos_engine.project_registry.RegisteredProject`). Used
    ...
    def __init__(
        ...
        # ring buffer.
        if state.snapshot_dir == Path.home() / ".pharos_engine" / "autosave":
    ...
    def _engine_version() -> str:
        """Return ``pharos_engine.__version__`` with a graceful fallback."""
        try:
