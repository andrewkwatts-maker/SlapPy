...
# in :mod:`pharos_engine.actions` so headless tests can import them
# directly without spinning up the DPG editor.
# ---------------------------------------------------------------------------


def _fb_save_project(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.project_actions import save_project
    return save_project(ctx)


def _fb_new_project(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.project_actions import new_project
    return new_project(ctx)


def _fb_open_recent(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.project_actions import open_recent
    return open_recent(ctx)


def _fb_view_reset_layout(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_actions import reset_layout
    return reset_layout(ctx)


def _fb_duplicate_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_actions import duplicate_selection
...
# See :mod:`pharos_engine.actions.selection_actions` and
# :mod:`pharos_engine.actions.theme_actions` for the implementations.
# ---------------------------------------------------------------------------


def _fb_select_all(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_actions import select_all
    return select_all(ctx)


def _fb_deselect_all(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_actions import deselect_all
    return deselect_all(ctx)


def _fb_copy_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_actions import copy_selection
    return copy_selection(ctx)


def _fb_paste_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_actions import paste_selection
    return paste_selection(ctx)


def _fb_theme_cycle(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_actions import cycle_theme
...
# and active-theme export. See :mod:`pharos_engine.actions.tool_settings_actions`,
# :mod:`pharos_engine.actions.camera_actions`, and
# :mod:`pharos_engine.actions.theme_io_actions` for the implementations.
# ---------------------------------------------------------------------------
...
def _fb_zoom_in(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.camera_actions import zoom_in
    return zoom_in(ctx)


def _fb_zoom_out(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.camera_actions import zoom_out
    return zoom_out(ctx)


def _fb_zoom_reset(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.camera_actions import zoom_reset
    return zoom_reset(ctx)


def _fb_export_current_theme(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_io_actions import export_current_theme
...
# :mod:`pharos_engine.actions.destructive_edit_actions`,
# :mod:`pharos_engine.actions.viewport_framing_actions`, and
# :mod:`pharos_engine.actions.tool_mode_actions` for the implementations.
# ---------------------------------------------------------------------------


def _fb_cut_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.destructive_edit_actions import cut_selection
    return cut_selection(ctx)


def _fb_delete_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.destructive_edit_actions import delete_selection
    return delete_selection(ctx)


def _fb_center_on_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.viewport_framing_actions import (
        center_on_selection,
    )
...
def _fb_frame_all(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.viewport_framing_actions import frame_all
    return frame_all(ctx)


def _fb_activate_pan_tool(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.tool_mode_actions import activate_pan_tool
...
#
# Implementation: :mod:`pharos_engine.actions.camera_animation_actions`.
# ---------------------------------------------------------------------------
...
# See :mod:`pharos_engine.actions.theme_import_actions`,
# :mod:`pharos_engine.actions.layout_io_actions`, and
# :mod:`pharos_engine.actions.history_actions` for the implementations.
# ---------------------------------------------------------------------------


def _fb_theme_import_from_file(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_import_actions import import_from_file
    return import_from_file(ctx)


def _fb_save_layout_as(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layout_io_actions import save_layout_as
    return save_layout_as(ctx)


def _fb_load_layout_from_file(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layout_io_actions import load_layout_from_file
    return load_layout_from_file(ctx)


def _fb_edit_undo(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.history_actions import undo
    return undo(ctx)


def _fb_edit_redo(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.history_actions import redo
    return redo(ctx)
...
# :mod:`pharos_engine.actions.edit_by_name_actions`,
# :mod:`pharos_engine.actions.spawn_history_actions`,
# :mod:`pharos_engine.actions.view_toggle_actions`, and
# :mod:`pharos_engine.actions.content_shell_actions` for the implementations.
# ---------------------------------------------------------------------------
...
def _fb_toggle_grid(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_actions import toggle_grid
    return toggle_grid(ctx)


def _fb_toggle_gizmos(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_actions import toggle_gizmos
...
# See :mod:`pharos_engine.actions.layer_duplicate_actions`,
# :mod:`pharos_engine.actions.theme_cycle_reverse_actions`,
# :mod:`pharos_engine.actions.panel_visibility_actions`, and
# :mod:`pharos_engine.actions.spawn_batch_actions` for the implementations.
# ---------------------------------------------------------------------------


def _fb_duplicate_layer(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_duplicate_actions import duplicate_layer
...
def _fb_close_all_panels(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.panel_visibility_actions import close_all_panels
...
def _fb_repeat_last_batch(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_batch_actions import repeat_last_batch
...
# See :mod:`pharos_engine.actions.edit_group_actions`,
# :mod:`pharos_engine.actions.theme_random_actions`,
# :mod:`pharos_engine.actions.spawn_cursor_actions`, and
# :mod:`pharos_engine.actions.edit_snap_pixel_actions`.
# ---------------------------------------------------------------------------


def _fb_group_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_group_actions import group_selection
    return group_selection(ctx)


def _fb_ungroup_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_group_actions import ungroup_selection
...
def _fb_snap_to_pixel_grid(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_snap_pixel_actions import snap_to_pixel_grid
...
# :mod:`pharos_engine.actions.content_folder_actions`,
# :mod:`pharos_engine.actions.content_rename_actions`,
# :mod:`pharos_engine.actions.panel_close_others_actions`,
# :mod:`pharos_engine.actions.edit_select_children_actions`, and
# :mod:`pharos_engine.actions.theme_reload_actions` for the implementations.
# ---------------------------------------------------------------------------


def _fb_new_folder(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.content_folder_actions import new_folder
    return new_folder(ctx)


def _fb_rename_asset(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.content_rename_actions import rename_asset
    return rename_asset(ctx)


def _fb_close_other_panels(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.panel_close_others_actions import (
        close_other_panels,
    )
...
def _fb_select_children(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_children_actions import (
        select_children,
    )
...
def _fb_reload_all_themes(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_reload_actions import reload_all_themes
...
# :mod:`pharos_engine.actions.content_delete_actions`,
# :mod:`pharos_engine.actions.panel_layout_actions`,
# :mod:`pharos_engine.actions.edit_invert_selection_actions`, and
# :mod:`pharos_engine.actions.view_fullscreen_actions` for the
# implementations.
# ---------------------------------------------------------------------------


def _fb_delete_asset(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.content_delete_actions import delete_asset
    return delete_asset(ctx)


def _fb_tile_grid(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.panel_layout_actions import tile_grid
    return tile_grid(ctx)


def _fb_cascade_panels(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.panel_layout_actions import cascade
    return cascade(ctx)


def _fb_invert_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_invert_selection_actions import (
        invert_selection,
    )
...
def _fb_view_fullscreen(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_fullscreen_actions import fullscreen
...
# :mod:`pharos_engine.actions.edit_select_next_actions`,
# :mod:`pharos_engine.actions.edit_paste_original_actions`,
# :mod:`pharos_engine.actions.spawn_batch_row_actions`, and
# :mod:`pharos_engine.actions.content_duplicate_asset_actions`.
# ---------------------------------------------------------------------------


def _fb_select_next(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_next_actions import select_next
    return select_next(ctx)


def _fb_select_previous(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_next_actions import (
        select_previous,
    )
...
def _fb_paste_at_original_position(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_paste_original_actions import (
...
def _fb_spawn_batch_row(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_batch_row_actions import spawn_batch_row
    return spawn_batch_row(ctx)


def _fb_duplicate_asset(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.content_duplicate_asset_actions import (
        duplicate_asset,
    )
...
# :mod:`pharos_engine.actions.edit_hide_show_actions`,
# :mod:`pharos_engine.actions.edit_lock_unlock_actions`, and
# :mod:`pharos_engine.actions.edit_select_by_kind_actions`.
# ---------------------------------------------------------------------------
...
def _fb_lock_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_lock_unlock_actions import lock_selection
...
# See :mod:`pharos_engine.actions.edit_mirror_actions`,
# :mod:`pharos_engine.actions.view_orbit_actions`, and
# :mod:`pharos_engine.actions.view_snap_actions`.
# ---------------------------------------------------------------------------


def _fb_mirror_selection_x(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_mirror_actions import mirror_selection_x
    return mirror_selection_x(ctx)


def _fb_mirror_selection_y(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_mirror_actions import mirror_selection_y
    return mirror_selection_y(ctx)


def _fb_mirror_selection_z(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_mirror_actions import mirror_selection_z
...
# :mod:`pharos_engine.actions.view_frame_selected_actions`,
# :mod:`pharos_engine.actions.view_reset_view_actions`,
# :mod:`pharos_engine.actions.panel_dock_actions`, and
# :mod:`pharos_engine.actions.theme_hot_swap_actions`.
# ---------------------------------------------------------------------------


def _fb_frame_selected(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_frame_selected_actions import frame_selected
    return frame_selected(ctx)


def _fb_reset_view(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_reset_view_actions import reset_view
    return reset_view(ctx)


def _fb_dock_left(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.panel_dock_actions import dock_left
    return dock_left(ctx)


def _fb_dock_right(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.panel_dock_actions import dock_right
    return dock_right(ctx)


def _fb_theme_hot_swap(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_hot_swap_actions import hot_swap
    return hot_swap(ctx)
...
# Python fallback lives in ``pharos_engine.actions.layer_solo_actions``,
...
def _fb_layer_solo(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_solo_actions import solo_layer
    return solo_layer(ctx)


def _fb_layer_merge_down(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_merge_down_actions import merge_down
    return merge_down(ctx)


def _fb_selection_grow(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_grow_actions import grow_selection
    return grow_selection(ctx)


def _fb_snap_increase_grid_size(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_grid_size_actions import increase_grid_size
    return increase_grid_size(ctx)


def _fb_snap_decrease_grid_size(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_grid_size_actions import decrease_grid_size
...
# Python fallback lives in ``pharos_engine.actions.selection_shrink_actions``,
...
def _fb_selection_shrink(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_shrink_actions import shrink_selection
    return shrink_selection(ctx)


def _fb_selection_invert_by_type(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_invert_by_type_actions import (
        invert_by_type,
    )
...
def _fb_view_toggle_wireframe(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_wireframe_actions import (
        toggle_wireframe,
    )
...
def _fb_edit_rename(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_rename_actions import rename_entity
    return rename_entity(ctx)


def _fb_edit_duplicate_at_cursor(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_duplicate_at_cursor_actions import (
        duplicate_at_cursor,
    )
...
# Python fallback lives in ``pharos_engine.actions.spawn_origin_actions``,
...
def _fb_spawn_at_origin(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_origin_actions import spawn_at_origin
    return spawn_at_origin(ctx)


def _fb_selection_by_type(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_by_type_actions import select_by_type
    return select_by_type(ctx)


def _fb_selection_by_layer(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_by_layer_actions import (
        select_by_layer,
    )
...
def _fb_selection_same_material(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_same_material_actions import (
...
def _fb_view_toggle_stats(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_stats_actions import toggle_stats
...
# ``pharos_engine.actions.edit_select_similar_actions``,
...
def _fb_edit_select_similar(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_similar_actions import (
        select_similar,
    )
...
def _fb_theme_reset_to_default(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_reset_default_actions import (
        reset_to_default,
    )
...
def _fb_layer_hide_others(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_hide_others_actions import hide_others
    return hide_others(ctx)


def _fb_layer_isolate(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_isolate_actions import isolate
    return isolate(ctx)


def _fb_snap_toggle_incremental(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_toggle_incremental_actions import (
        toggle_incremental,
    )
...
# in ``pharos_engine.actions.content_reveal_explorer_actions``,
...
def _fb_content_reveal_in_explorer(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.content_reveal_explorer_actions import (
        reveal_in_explorer,
    )
...
def _fb_content_duplicate_folder(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.content_duplicate_folder_actions import (
        duplicate_folder,
    )
...
def _fb_view_increase_pixel_scale(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_pixel_scale_actions import (
...
def _fb_spawn_stamp_repeat(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_stamp_repeat_actions import (
        stamp_repeat,
    )
...
# live in ``pharos_engine.actions.view_set_zoom_actions``,
...
def _fb_view_set_zoom(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_set_zoom_actions import set_zoom
    return set_zoom(ctx)


def _fb_spawn_at_view_center(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_view_center_actions import (
...
def _fb_spawn_stamp_random(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_stamp_random_actions import (
        stamp_random,
    )
...
def _fb_theme_reload_from_disk(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.theme_reload_from_disk_actions import (
        reload_from_disk,
    )
...
def _fb_layer_rename(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_rename_actions import rename_layer
...
# ``pharos_engine.actions.spawn_origin_offset_actions``,
...
def _fb_spawn_at_origin_offset(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_origin_offset_actions import (
...
def _fb_edit_flatten_selection(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_flatten_selection_actions import (
        flatten_selection,
    )
...
def _fb_snap_set_angle_snap(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_angle_snap_actions import set_angle_snap
    return set_angle_snap(ctx)


def _fb_layer_move_up(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_reorder_actions import move_layer_up
    return move_layer_up(ctx)


def _fb_layer_move_down(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_reorder_actions import move_layer_down
...
# Python fallbacks live in
# ``pharos_engine.actions.layer_lifecycle_actions``,
...
def _fb_layer_new(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_lifecycle_actions import create_layer
    return create_layer(ctx)


def _fb_layer_delete(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_lifecycle_actions import delete_layer
    return delete_layer(ctx)


def _fb_snap_set_grid_size(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_set_grid_size_actions import set_grid_size
    return set_grid_size(ctx)


def _fb_view_toggle_ruler(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_ruler_actions import toggle_ruler
    return toggle_ruler(ctx)


def _fb_spawn_at_last_position(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_last_position_actions import (
...
# Python fallbacks live in
# ``pharos_engine.actions.view_toggle_axes_actions``,
...
def _fb_view_toggle_axes(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_axes_actions import toggle_axes
    return toggle_axes(ctx)


def _fb_view_toggle_background(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_background_actions import (
        toggle_background,
    )
...
def _fb_edit_select_by_tag(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.selection_by_tag_actions import select_by_tag
    return select_by_tag(ctx)


def _fb_spawn_at_grid(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_at_grid_actions import spawn_at_grid
    return spawn_at_grid(ctx)


def _fb_layer_clear(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_clear_actions import clear_layer
...
def _fb_view_toggle_snap_indicator(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_snap_indicator_actions import (
...
def _fb_edit_select_parent(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_parent_actions import select_parent
    return select_parent(ctx)


def _fb_spawn_at_selection_center(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_at_selection_center_actions import (
...
def _fb_layer_lock(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_lock_actions import toggle_layer_lock
    return toggle_layer_lock(ctx)


def _fb_snap_reset_defaults(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_reset_defaults_actions import (
        reset_snap_defaults,
    )
...
def _fb_view_toggle_safe_area(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_safe_area_actions import (
        toggle_safe_area,
    )
...
def _fb_edit_select_root(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_root_actions import select_root
    return select_root(ctx)


def _fb_spawn_at_last_click(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_at_last_click_actions import (
        spawn_at_last_click,
    )
...
def _fb_layer_unlock_all(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_unlock_all_actions import (
        unlock_all_layers,
    )
...
def _fb_snap_cycle_grid_size(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_cycle_grid_size_actions import (
        cycle_grid_size,
    )
...
def _fb_view_toggle_camera_bounds(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.view_toggle_camera_bounds_actions import (
...
def _fb_edit_select_last_spawned(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.edit_select_last_spawned_actions import (
        select_last_spawned,
    )
...
def _fb_spawn_at_previous_click(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.spawn_at_previous_click_actions import (
...
def _fb_layer_sort_by_z(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.layer_sort_by_z_actions import sort_by_z
    return sort_by_z(ctx)


def _fb_snap_toggle_pixel_perfect(ctx: dict[str, Any]) -> Any:
    from pharos_engine.actions.snap_toggle_pixel_perfect_actions import (
