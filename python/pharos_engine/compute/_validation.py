"""Internal input-validation helpers for the ``pharos_engine.compute`` and
``pharos_engine.post_process`` GPU dispatch paths (hardening round 14).

Generic validators (``validate_str``, ``validate_positive_int`` …) live in
:mod:`pharos_engine._validation`. The dispatch- and shader-loader-specific
