...
from ``pharos_engine.physics`` so it can survive Phase D's strip pass and
serve as the canonical Poisson solver going forward.

Algorithm provenance
--------------------
Lifted from the working core of
``pharos_engine.physics.pressure_multigrid``'s ``vcycle_project_v`` so
...
plan as ``pharos_engine.dynamics``.
"""
