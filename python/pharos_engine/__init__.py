...
Quickstart
----------
>>> from pharos_engine import Engine, Scene, Entity, Camera
...
* :mod:`pharos_engine.softbody` — XPBD lattice + beam softbody solver
  (lattice / vehicle / humanoid body builders, ``World.step``).
* :mod:`pharos_engine.fluid`    — PBF fluid (``FluidWorld``,
  ``apply_fluid_buoyancy``, surface extraction).
* :mod:`pharos_engine.dynamics` — unified ``Body`` / ``Material`` /
  ``JointSpec`` / ``RagdollSpec`` / ``IKChainSpec`` over the XPBD substrate.
* :mod:`pharos_engine.physics`  — legacy per-pixel hierarchical-hull solver
  (compat surface; new code prefers ``softbody`` / ``fluid``).
* :mod:`pharos_engine.thermal`  — ``HeatField`` Laplacian + two-region
  exchange for cross-system heat transport.
* :mod:`pharos_engine.topology` — connected-components labelling for
  fragmentation / fracture (``connected_components``, ``_grid``).
* :mod:`pharos_engine.numerics` — V-cycle Poisson solver
  (``vcycle_poisson``, ``sor_smooth``, ``compute_residual``).
* :mod:`pharos_engine.math`     — symbolic + numeric formula evaluation
  ...
  ``Vec2`` / ``Vec3`` / ``Vec4``.
* :mod:`pharos_engine.zones`    — generic ``RectZone`` / ``ThresholdZone``
  / ``ZoneManager`` (pickup, spawn pad, damage zone).

Rendering + GPU:

* :mod:`pharos_engine.gpu`         — wgpu context, mesh / PBR / cluster
  pipelines, SDF renderer.
* :mod:`pharos_engine.gi`          — radiance cascades, ReSTIR, SVGF.
* :mod:`pharos_engine.post_process` — TAA, GTAO, bloom, DoF, tonemap,
  shadow CSM, volumetric fog, preset chains.
* :mod:`pharos_engine.material`    — node-graph material authoring
  (``NodeMaterial`` + factory nodes, ``MaterialMap``).
* :mod:`pharos_engine.compute`     — ``ComputePass`` / ``ComputePipeline``,
  stats / spatial / mutator helpers, ``AssetComputeAPI``.
* :mod:`pharos_engine.residency`   — three-tier (GPU/RAM/DISK) residency
  + ``.slap`` binary format.

Authoring + tooling:

* :mod:`pharos_engine.studio`     — ``softbody_stage`` / ``fluid_stage`` /
  ``humanoid_stage`` + ``record()`` GIF capture (15-line demo helpers).
* :mod:`pharos_engine.iso`        — isometric grid + scene + combat
...
* :mod:`pharos_engine.testing`    — ``assert_scene_matches`` golden-frame
  visual diff harness.
...
# ---------------------------------------------------------------------------
from pharos_engine.config import engine_config  # noqa: E402
...
def __getattr__(name: str):
    ...
    _subpackages = {
        ...
        # ``from pharos_engine import zones`` returns the module itself.
        # See ``pharos_engine.zones.RectZone`` / ``ThresholdZone`` /
...
#
# Eager import so ``import pharos_engine; pharos_engine.launch(...)`` works
...
# ---------------------------------------------------------------------------
from pharos_engine.app import (  # noqa: E402
    App,
    AppConfig,
    ModelHandle,
    TextureHandle,
...
# ---------------------------------------------------------------------------
from pharos_engine.diagnostics import (  # noqa: E402
    DiagnosticEvent,
