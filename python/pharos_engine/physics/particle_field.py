...
2. **Per-pixel collision mask.** :attr:`ParticleField.mask` is an
   ...
   "top".
3. **Region-based scaling.** A :class:`~pharos_engine.physics.baked_terrain.RegionGrid`
...
:mod:`pharos_engine.fluid.pbf` solver via :meth:`ParticleField.step` when
...
from pharos_engine.physics.splat import SplatConfig, compute_splat
from pharos_engine.physics.fluid_bridge import FluidBridgeConfig, bridge_step
...
class ParticleField:
    ...
    # pharos_engine.physics.particle_gpu. Default OFF so existing tests
    ...
    # kernel in pharos_engine.physics.particle_gpu_drill. Default OFF.
    ...
    # in pharos_engine.physics.particle_gpu (gpu_thermal_step). Default
    ...
    # pharos_engine.physics.particle_gpu (gpu_collide). Default OFF;
    ...
    # pharos_engine.physics.particle_gpu (gpu_slide). Default OFF.
    ...
    # pharos_engine.physics.particle_gpu (gpu_slump_loose). Default OFF.
    ...
    # ``pharos_engine.physics.particle_gpu`` (``gpu_bake_settled``).
    ...
    def step(self, dt: float) -> None:
        ...
        if air_mask.any():
            if self.use_gpu_integrate:
                ...
                from pharos_engine.physics.particle_gpu import gpu_integrate
            ...
            if self.use_gpu_drill:
                ...
                from pharos_engine.physics.particle_gpu_drill import (
                    gpu_drill_through,
                )
            ...
            if air_mask.any():
                if self.use_gpu_collide:
                    ...
                    from pharos_engine.physics.particle_gpu import gpu_collide
                    gpu_collide(self, dt)
                else:
        ...
        # (pharos_engine.fluid.pbf_step via physics.fluid_bridge). Replaces
        ...
        if slide_mask.any():
            if self.use_gpu_slide:
                # Lazy import — avoids paying the wgpu cost when the
                # GPU path is disabled (the default).
                from pharos_engine.physics.particle_gpu import gpu_slide
                gpu_slide(self, dt)
            else:
        ...
        if self.use_gpu_bake:
            from pharos_engine.physics.particle_gpu import gpu_bake_settled
        ...
        if self.use_gpu_slump:
            from pharos_engine.physics.particle_gpu import gpu_slump_loose
