...
from pharos_engine._compat import cell_material_for
from pharos_engine.physics.body import (
    make_circle_silhouette,
    make_rect_silhouette,
)
from pharos_engine.physics.hull import TIER_T0, TIER_T1, TIER_T2
from pharos_engine.physics.world import PhysicsWorld, load_physics_config
...
def _load_silhouette_png(path: str | Path) -> np.ndarray:
    ...
    try:
        ...
        raise ImportError(
            "custom_silhouette shapes require Pillow; install pharos-engine "
            "with image support."
        ) from exc
    p = Path(path)
    if not p.exists():
