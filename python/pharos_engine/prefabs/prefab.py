...
:class:`pharos_engine.dynamics.World`. Each prefab bundles a *body spec*
(the shape of one or more :class:`~pharos_engine.dynamics.body.Body`
handles), a list of :class:`~pharos_engine.dynamics.joint.JointSpec`
...
class Prefab:
    ...
    joint_specs:
        List of :class:`~pharos_engine.dynamics.joint.JointSpec` dicts.
    ...
    def spawn(
        ...
        *position* (rotated by *rotation* radians when the body kind
        ...
        -------
        list[Body]
            Every :class:`~pharos_engine.dynamics.body.Body` handle the
...
def _spawn_body_spec(
    ...
    * ``rope``      — passes through
      :func:`pharos_engine.dynamics.build_rope` using ``node_count`` /
      ``total_length`` from the spec.
    * ``ragdoll``   — passes through
      :func:`pharos_engine.dynamics.build_ragdoll` using the ``bones``
      list from the spec.
    ...
    * ``composite`` — creates a bare :class:`Body` marker only; the
    ...
    from pharos_engine.dynamics.ragdoll import BoneSpec
