...
* :attr:`PrefabLibrary.BAKED_DIR` — read-only ``*.prefab.yaml`` shipped
  inside the wheel at ``python/pharos_engine/prefabs/baked/``.
* :attr:`PrefabLibrary.USER_DIR` — writable location under
  ``~/.pharos_engine/prefabs/``. :meth:`PrefabLibrary.bake_defaults`
...
class PrefabLibrary:
    ...
    USER_DIR: Path = Path.home() / ".pharos_engine" / "prefabs"
