"""pharos_engine._core_facade — Rust-backend bypass facade (HH8, 2026-07-05).
...
PyO3 extension :mod:`pharos_engine._core` (built from ``src/*.rs`` via
...
* The compiled ``_core`` module is **flat** — every ``#[pyfunction]`` and
  ...
  (``pharos_engine._core.hull``, ``.raster``, ``.pbf_solver`` …) that view
...
class _NullCore:
    """Attribute-access stub used when :mod:`pharos_engine._core` is missing.
    ...
    __name__ = "pharos_engine._core (unavailable)"

    def __repr__(self) -> str:  # pragma: no cover - trivial
        return "<_NullCore: pharos_engine._core extension not compiled>"

    def __getattr__(self, name: str) -> Any:
        raise _NullCoreError(
            f"pharos_engine._core.{name} is unavailable — the Rust "
            "extension was not compiled for this install. Build with: "
            "``maturin develop --release`` from the repo root, or "
            "``pip install pharos_engine`` on a supported platform."
        )
...
try:
    from pharos_engine import _core as _real_core  # type: ignore[import-not-found]
    _HAS_NATIVE = True
    core: Any = _real_core
except ImportError:
    _HAS_NATIVE = False
...
# ---------------------------------------------------------------------------
# Synthetic sub-module views under pharos_engine._core.<name>
# ---------------------------------------------------------------------------
#
...
def _install_submodule_views() -> None:
    """Populate ``sys.modules['pharos_engine._core.<name>']`` for each
    logical Rust module whose symbols are present at runtime."""
    if not _HAS_NATIVE:
        return
    parent_name = _real_core.__name__  # "pharos_engine._core"
    for mod, meta in RUST_MODULE_MAP.items():
        present = [s for s in meta["symbols"] if hasattr(_real_core, s)]
        if not present:
            continue
        ...
        shim.__doc__ = (
            ...
            f"{meta['summary']}\n\n"
            "Auto-generated by pharos_engine._core_facade — the compiled "
        ...
        # ``from pharos_engine._core import raster`` resolves via the
