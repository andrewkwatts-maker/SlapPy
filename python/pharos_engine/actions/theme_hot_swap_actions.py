...
Backs the ``theme.hot_swap`` :class:`~pharos_engine.tool_router.ToolAction`
...
it against :func:`pharos_engine.ui.theme.get_theme` before applying via
:func:`~pharos_engine.ui.theme.apply_theme`.
...
the ``pharos_engine.ui.theme`` package directly.
...
def hot_swap(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if theme_mod is None:
        return {
            "status": "error",
            "message": "pharos_engine.ui.theme failed to import",
        }
