...
Backs the ``theme.random`` :class:`~pharos_engine.tool_router.ToolAction`
row added by the EE1 STUB-triage sprint tick (round 8 after
X3 / Y1 / Z7 / AA1 / BB1 / CC1 / DD1).

Sibling to :func:`pharos_engine.actions.theme_actions.cycle_theme` and
:func:`pharos_engine.actions.theme_cycle_reverse_actions.cycle_theme_reverse`.
Prefers a shell hook when present (``shell.set_theme(name)`` /
``shell.apply_theme(name)``), otherwise walks
:func:`pharos_engine.ui.theme.list_registered_themes` and picks a random
