...
Backs the ``theme.reload_all`` :class:`~pharos_engine.tool_router.ToolAction`
...
2. Reset the shared theme cursor
   (:mod:`pharos_engine.actions.theme_actions._THEME_CURSOR`) so the
   next ``theme.cycle`` starts from a clean slate.
3. Clear the process-wide ``_REGISTRY`` via
   :func:`pharos_engine.ui.theme._reset_registry_for_tests` (an
   internal-but-reused escape hatch — the shipping name is misleading,
   the function is our stable reset point).
4. Re-run :func:`pharos_engine.ui.theme.bake_default_themes` to re-seed
   the builtin roster.
...
* ``{"status": "error", "message": "<...>"}`` when the ui.theme
  ...
  since the tests import :mod:`pharos_engine.tool_router` which in turn
