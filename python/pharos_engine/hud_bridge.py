...
class _HUDStubRenderer:
    ...
    * ``submit_text(mesh, color)`` (optional — text is skipped when absent
    ...
    The HH1 :class:`~pharos_engine.app._StubRenderer` implements none of
...
# ---------------------------------------------------------------------------
# Camera2D shim — used when :mod:`pharos_engine.render.camera` is unavailable
...
def _build_camera_2d(app: Any) -> Any:
    """Return a :class:`~pharos_engine.render.Camera2D` bound to app's viewport.
...
def mount_hud(app: Any, *, widgets: Iterable[Any] | None = None) -> Any:
    ...
    * ``before_frame_render(app)`` → ``hud.submit_to_renderer()`` — pushes
    ...
    ----------
    app:
        The :class:`~pharos_engine.App` instance to attach to.
    widgets:
