...
The CLI entry point lives at :mod:`pharos_engine.perf.cli` and is spelt
``python -m pharos_engine.perf.tripwire``.

Public symbols are re-exported lazily via ``__getattr__`` so that
``python -m pharos_engine.perf.tripwire`` does not trip Python's
