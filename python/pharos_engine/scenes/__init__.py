"""``pharos_engine.scenes`` — YAML scene serialisation (FF3).

A *scene* is a lightweight authoring document that describes the
entities placed into a :class:`pharos_engine.dynamics.World`. Each scene
round-trips through YAML (``*.scene.yaml``) so hand-editing, VCS diffs,
and headless editor tooling all use the same on-disk shape.

Two-line usage::

    from pharos_engine.scenes import Scene, SceneFile, SceneRegistry
    scene = SceneFile.read("levels/pit.scene.yaml")

Module layout
~~~~~~~~~~~~~

* :mod:`pharos_engine.scenes.scene` — :class:`Scene` dataclass with
  :meth:`Scene.add_entity` / :meth:`Scene.apply_to_world` /
  :meth:`Scene.snapshot_from_world` / :meth:`Scene.to_yaml`.
* :mod:`pharos_engine.scenes.scene_file` — :class:`SceneFile` static
  read/write/validate helpers with atomic writes.
* :mod:`pharos_engine.scenes.scene_registry` — :class:`SceneRegistry`
  filesystem discovery + in-memory cache.

The subpackage is deliberately named ``scenes`` (plural) to avoid
shadowing the runtime :mod:`pharos_engine.scene` module, which owns the
live :class:`~pharos_engine.scene.Scene` (entities, event bus, GPU state).
