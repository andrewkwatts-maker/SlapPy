...
This module wires the ergonomic HH1 :class:`pharos_engine.app.App` API to
the HH4 renderer (:mod:`pharos_engine.render`) and the HH5 asset importer
(:mod:`pharos_engine.asset_import`). Nothing else in the engine needs
to know about the three surfaces at once — the bridge lives here so
``import pharos_engine`` remains cheap.

The four public helpers:

* :func:`bridge_load_model` — parses an asset via HH5, builds a proper
  :class:`pharos_engine.render.mesh.Mesh`, records it on a fresh
  :class:`~pharos_engine.app.ModelHandle`, and returns the handle.
...
* :func:`promote_stub_renderer` — swaps the HH1 logging stub for a
  real HH4 :class:`~pharos_engine.render.renderer.Renderer` (or
  :class:`~pharos_engine.render.null_renderer.NullRenderer`) at runtime.
* :func:`default_material` — a sensible starting :class:`Material`
  (opaque, light gray).

The module deliberately avoids any hard import of
:mod:`pharos_engine.render` or :mod:`pharos_engine.asset_import` at
...
if TYPE_CHECKING:  # pragma: no cover - import only for type hints
    from pharos_engine.app import App, LightHandle, ModelHandle
...
def _extract_positions_and_indices(imported: Any) -> tuple[np.ndarray, np.ndarray]:
    ...
    Handles three shapes emitted by :mod:`pharos_engine.asset_import`:
...
def _to_render_mesh(imported: Any) -> "Mesh":
    """Build a :class:`pharos_engine.render.mesh.Mesh` from an HH5 import.
    ...
    """
    from pharos_engine.render.mesh import Mesh  # noqa: PLC0415 - soft dep
...
def default_material() -> "Material":
    ...
    """
    from pharos_engine.render.material import Material  # noqa: PLC0415 - soft dep
...
def bridge_load_model(app: "App", path: str) -> "ModelHandle":
    ...
    * Is appended to ``app.models`` (matches HH1 :meth:`App.load_model`
      semantics).
    * Has ``.mesh`` set to a :class:`pharos_engine.render.mesh.Mesh`.
    ...
    * Has ``.bounding_box`` recorded ``((min_xyz), (max_xyz))`` for
    ...
    """
    from pharos_engine.app import ModelHandle  # noqa: PLC0415 - avoid import cycle

    try:
        from pharos_engine.asset_import import import_asset  # noqa: PLC0415
...
def bridge_submit_frame(app: "App", renderer: Any) -> None:
    ...
    if cam is not None:
        try:
            from pharos_engine.render.camera import Camera3D  # noqa: PLC0415
    ...
    if app.lights:
        try:
            from pharos_engine.render.light import Light  # noqa: PLC0415
...
def promote_stub_renderer(app: "App") -> None:
    ...
    """
    from pharos_engine.app import _StubRenderer  # noqa: PLC0415

    if not isinstance(app._renderer, _StubRenderer):
        return  # already promoted

    try:
        from pharos_engine.render import (  # noqa: PLC0415
            NullRenderer,
            Renderer,
            is_wgpu_available,
        )
