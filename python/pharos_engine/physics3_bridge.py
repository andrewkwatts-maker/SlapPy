...
``pharos_engine.physics`` tree. This module gives them a stable
Python-level surface with two implementations:

* ``backend="physics"`` — delegates to :mod:`pharos_engine.physics`
...
The soft-import contract mirrors what :mod:`pharos_engine.render.bvh_3d`
...
The 2D dynamics surface (``pharos_engine.dynamics.World`` and
``pharos_engine.dynamics.Body``) shaped this API: game code that
...
try:  # pragma: no cover - the WIP tree may or may not be importable
    from pharos_engine import physics as _physics_pkg  # noqa: F401
    _HAS_PHYSICS = True
...
try:
    from pharos_engine.render.bvh_3d import AABB3D as _AABB3D  # noqa: F401
    from pharos_engine.render.bvh_3d import BVH3D as _BVH3D  # noqa: F401
    _HAS_AABB3D = True
    _HAS_BVH3D = True
...
def resolve_physics3_backend() -> str:
    ...
    * ``"physics"`` — :mod:`pharos_engine.physics` imported cleanly.
...
class World3D:
    ...
    * ``backend`` — the tag returned by :func:`resolve_physics3_backend`
        ...
        if requested == "physics":
            if not _HAS_PHYSICS:
                raise PhysicsBackendError(
                    "backend='physics' requested but pharos_engine.physics"
                    " is not importable"
                )
            return "physics"
    ...
    def build_bvh(self) -> None:
        ...
        :class:`PhysicsBackendError` if :mod:`pharos_engine.render.bvh_3d`
        ...
        if not _HAS_BVH3D:
            raise PhysicsBackendError(
                "World3D.build_bvh: pharos_engine.render.bvh_3d is not"
    ...
    def _step_physics(self, dt: float) -> None:
        """Delegate to the WIP ``pharos_engine.physics`` tree.
        ...
        API. This matches how :mod:`pharos_engine.render.bvh_3d` used a
...
# ``pharos_engine.render.bvh_3d`` themselves. When missing, expose the
