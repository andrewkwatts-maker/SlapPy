"""SceneWalker — traverse an FF3 :class:`~pharos_engine.scenes.scene.Scene`
...
* Asset loads route through :mod:`pharos_engine.asset_import` when a
  ``params["mesh_path"]`` is present; results are memoised in
  :class:`AssetCache` so the walker never re-hits disk mid-frame.
* Prefab resolution soft-imports :class:`pharos_engine.prefabs.PrefabLibrary`
...
class SceneWalker:
    ...
    ----------
    scene
        The read-only :class:`~pharos_engine.scenes.scene.Scene` to walk.
        The walker never mutates the scene.
    prefab_library
        Optional :class:`pharos_engine.prefabs.PrefabLibrary`. When
    ...
    def _resolve_mesh_from_path(self, mesh_path: str) -> Mesh | None:
        ...
        if cached is not None:
            return cached
        try:
            from pharos_engine.asset_import import import_asset
        except Exception as exc:  # pragma: no cover - defensive
            _LOG.warning(
