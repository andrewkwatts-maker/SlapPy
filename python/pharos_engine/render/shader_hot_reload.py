...
* **Every reload emits a bus event.** ``shader.reloaded`` on
  :mod:`pharos_engine.event_bus` — subscribers (a running renderer, a
...
from pharos_engine import event_bus as _event_bus
...
def get_default_reloader() -> ShaderHotReloader:
    ...
    pharos_engine.render.shader_hot_reload`` doesn't force a wgpu device
