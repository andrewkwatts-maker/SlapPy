...
Numpy arrays are encoded the same way as :mod:`pharos_engine.dynamics.serialize`:
...
>>> from pharos_engine.serialize import SaveGame, save, load
...
- ``pharos_engine.thermal.HeatField`` (temperature grid + conductivity + diffusivity)
- ``pharos_engine.zones.RectZone`` / ``ThresholdZone`` / ``ZoneManager``
- ``pharos_engine.iso.combat.WaveSpec`` / ``WaveSchedule`` / ``Attacker`` / ``Defender``
- ``pharos_engine.telemetry`` history ring buffer
...
def _rectzone_from_dict(d: dict):
    from pharos_engine.zones import RectZone, ThresholdZone
...
def _attacker_from_dict(d: dict):
    from pharos_engine.iso.combat import Attacker
    return Attacker(
        pos=tuple(d["pos"]),
        damage=d["damage"],
        reach=d["reach"],
...
def _defender_from_dict(d: dict):
    from pharos_engine.iso.combat import Defender
    return Defender(
        pos=tuple(d["pos"]),
        hp=d["hp"],
...
def _savegame_to_dict(sg: SaveGame) -> dict:
    ...
    if sg.world is not None:
        from pharos_engine.dynamics.serialize import world_to_dict
...
def _savegame_from_dict(d: dict) -> SaveGame:
    ...
    if "world" in d:
        from pharos_engine.dynamics.serialize import world_from_dict
...
def to_dict(obj: Any) -> dict:
    ...
    Supported: :class:`pharos_engine.dynamics.World`, ``HeatField``, ``RectZone``
...
def from_dict(d: dict, kind: str | None = None) -> Any:
    ...
    if k == "WaveSchedule":
        from pharos_engine.iso.combat import WaveSchedule
