...
The user's directive (2026-07-04 HH1 sprint):
    ...
    A developer should be able to ``pip install pharos_engine`` and, in two or
...
class ModelHandle:
    ...
    # :func:`pharos_engine.app_integration.bridge_load_model`; stays
    ...
    def transform_matrix(self):
        ...
        Delegates to :mod:`pharos_engine.app_integration` so the app
...
class App:
    ...
    def load_model(self, path: str | Path) -> ModelHandle:
        ...
        :func:`pharos_engine.app_integration.bridge_load_model`. This
        returns a handle with a real :class:`pharos_engine.render.mesh.Mesh`
    ...
    def _load_via_asset_importer(self, path: str | Path) -> ModelHandle | None:
        """Soft-import ``pharos_engine.app_integration`` and call the bridge.
    ...
    def render_frame(self) -> None:
        ...
        :func:`pharos_engine.app_integration.promote_stub_renderer`),
    ...
    def get_bounding_box_of_all_models(
        ...
        :func:`pharos_engine.app_integration.bridge_load_model`) after
    ...
    # ------------------------------------------------------------------
    # MM2 — HUD integration (delegates to pharos_engine.hud_bridge).
    # ------------------------------------------------------------------
    def enable_hud(
        self,
        ...
        A thin one-liner around :func:`pharos_engine.hud_bridge.mount_hud`
        ...
        HUDOverlay
            ...
            ``self._hud_overlay``.
        """
        from pharos_engine.hud_bridge import mount_hud
    ...
    # helpers so a user can ``pip install pharos_engine`` and record /
    ...
    #
    # Thin one-liners over :mod:`pharos_engine.diagnostics`. Users wanting
    # to bypass this convenience layer (custom min_level per subsystem,
    # collector attached to a non-``pharos_engine`` logger, etc.) can call
    # :func:`pharos_engine.diagnostics.get_global_collector` directly
    ...
    def enable_diagnostics(
        ...
        :meth:`~pharos_engine.diagnostics.DiagnosticsCollector.install`
        so it subscribes to the ``pharos_engine`` root logger, and stashes
        ...
        :func:`pharos_engine.hud_bridge.add_diagnostics_widget` so
        ...
        -----
        Bypass hint: call
        ``pharos_engine.diagnostics.get_global_collector()`` directly
    ...
    def disable_diagnostics(self) -> dict[str, Any]:
        ...
        Calls
        :meth:`~pharos_engine.diagnostics.DiagnosticsCollector.uninstall`
    ...
    def get_diagnostics(self) -> Any:
        ...
        -----
        Bypass hint: use
        :func:`pharos_engine.diagnostics.get_global_collector`
