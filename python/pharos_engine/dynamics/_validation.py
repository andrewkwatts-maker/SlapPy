...
:mod:`pharos_engine._validation` and are wrapped under domain-friendly
...
def validate_world(fn: str, world: Any) -> None:
    ...
    if missing:
        raise TypeError(
            f"{fn}: world must be a pharos_engine.dynamics.World "
...
def validate_body(name: str, fn: str, value: Any) -> Any:
    ...
    if not isinstance(value, _Body):
        raise TypeError(
            f"{fn}: {name} must be a pharos_engine.dynamics.Body; "
...
def validate_joint(name: str, fn: str, value: Any) -> Any:
    ...
    if not isinstance(value, _JointSpec):
        raise TypeError(
            f"{fn}: {name} must be a pharos_engine.dynamics.JointSpec; "
            f"got {type(value).__name__}"
        )
    return value
