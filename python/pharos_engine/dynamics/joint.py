...
Each builder in :mod:`pharos_engine.dynamics.spring` etc. is the *only* code
...
def resolve_joint_specs(world: WorldLike, specs: list[JointSpec]) -> list[int]:
    ...
    * If ``world`` exposes a softbody-style ``beams`` SoA (the
      ``pharos_engine.softbody.SoftBodyWorld`` duck), distance-flavoured
    ...
    raise TypeError(
        f"resolve_joint_specs: world must be a pharos_engine.dynamics.World "
