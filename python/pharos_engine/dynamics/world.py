...
class World:
    ...
    def _check_overdamping(self) -> None:
        ...
        for joint in self.joints:
            ...
            if effective > OVERDAMPING_THRESHOLD:
                ...
                warnings.warn(
                    (
                        f"pharos_engine.dynamics: joint id={id(joint)} "
    ...
    def step(self, dt: float) -> None:
        ...
        2. Iterate the joint list ``solver_iterations`` times, calling
           :func:`pharos_engine.dynamics.joint.resolve` to project each
           constraint.
...
# softbody-style world (e.g. ``pharos_engine.softbody.SoftBodyWorld``).
#
...
#   at module scope: ``pharos_engine.softbody.step(world)``).
#
...
class WorldLike(Protocol):
    ...
    * :class:`pharos_engine.dynamics.World` (this module) — the XPBD
      substrate. Has ``positions: np.ndarray``, ``step(dt)``, ``gravity``.
    * :class:`pharos_engine.softbody.SoftBodyWorld` — has ``nodes.pos`` and
      ``gravity``; stepping goes through the module-level
      ``pharos_engine.softbody.step(world)`` function.
    ...
    * :func:`resolve_joint_specs` dispatches by ``isinstance(world,
      ...
      softbody path.
    * :func:`pharos_engine.studio.dynamics_stage` only accepts
...
class DynamicsWorldLike(WorldLike, Protocol):
    ...
    :func:`pharos_engine.studio.dynamics_stage`'s default PIL renderer
