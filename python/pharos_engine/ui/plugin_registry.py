...
into ``~/.pharos_engine/extensions/<plugin_name>/`` and the engine will
...
def default_plugin_dir() -> Path:
    """Return ``~/.pharos_engine/extensions`` (does not create it)."""
    return Path.home() / ".pharos_engine" / "extensions"
...
def _import_entry(manifest: PluginManifest) -> Any:
    ...
    * A relative filename (``hello_plugin.py`` or
    ...
    if entry.endswith(".py") or "/" in entry or "\\" in entry:
        ...
        module_name = f"_pharos_engine_plugin_{manifest.name}"
...
class PluginRegistry:
    ...
    Typical use::
        ...
        registry.load_all()             # picks up ~/.pharos_engine/extensions
        ...
