...
The :mod:`pharos_engine.ui.user_overrides` layer (X6) already loads user
YAML hotkey files from ``~/.pharos_engine/ui/hotkeys/`` and folds them
into :class:`~pharos_engine.ui.editor.notebook_hotkeys.NotebookHotkeys`.
...
:data:`pharos_engine.tool_router.REGISTRY`, a first-launch
:func:`bake_defaults` bootstrapper, and a :class:`HotkeyRemapWatcher`
that subclasses the X6 :class:`~pharos_engine.ui.user_overrides.WatcherHandle`
...
* Hard boundary: this module never imports from
  :mod:`pharos_engine.ui.user_overrides` at module scope. The subclassed
...
class HotkeyBinding:
    ...
    action_id:
        The :class:`~pharos_engine.tool_router.ToolAction` identifier the
    ...
    def validate(self, router: Any) -> list[str]:
        ...
        if not callable(has_action):
            raise TypeError(
                ...
                "did you forget to pass pharos_engine.tool_router.REGISTRY?"
            )
        seen: set[str] = set()
        unknown: list[str] = []
        for b in self._bindings.values():
...
def default_hotkey_map() -> HotkeyMap:
    ...
    Imports :mod:`pharos_engine.ui.editor.notebook_hotkeys` lazily so this
...
class HotkeyRemapWatcher:
    ...
    Wraps :class:`pharos_engine.ui.user_overrides.UserOverrideLoader.watch_dir`
