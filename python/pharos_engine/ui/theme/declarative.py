...
into the existing :class:`~pharos_engine.ui.theme.theme_spec.ThemeSpec`
runtime.
...
* **Python-interpolation.**  Values may embed ``${expr}`` fragments.
  Each fragment is fed to :func:`pharos_engine.math.evaluate` (sandboxed
...
* **Cache-friendly.**  :func:`load_declarative` writes a parsed marker
  into ``~/.pharos_engine/themes/`` so repeat launches skip the parse
...
def _evaluate_interpolation(payload: str) -> Any:
    ...
    from pharos_engine.math import evaluate as _sandbox_evaluate
    try:
...
def load_declarative(path: str | Path) -> str:
    ...
    ``~/.pharos_engine/themes/<name>.cache.json`` as an advisory cache
    ...
    try:
        cache_dir = Path(os.path.expanduser("~/.pharos_engine/themes"))
