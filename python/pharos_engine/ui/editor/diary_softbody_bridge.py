...
the diary tick's ``from pharos_engine.softbody import step`` fails on a
fresh checkout because ``python/pharos_engine/softbody/`` is
...
* :func:`resolve_softbody_class` — return the constructor the diary
  ...
  ``pharos_engine.softbody.SoftBodyWorld`` when it is importable so a
  wheel-shipped or dev-checkout engine keeps working; falls back to
  the always-tracked ``pharos_engine.dynamics.SoftBodyWorld``. Raises
...
* :func:`import_softbody_file` — read a ``.softbody.yaml`` or
  ...
  :class:`pharos_engine.dynamics.Body` (or the softbody-native body
...
_MISSING_MESSAGE = (
    ...
    "'pharos_engine.softbody.SoftBodyWorld' (WIP) nor "
    "'pharos_engine.dynamics.SoftBodyWorld' (tracked fallback) could "
    "be imported. Install the engine wheel or run from a checkout "
    "that includes 'python/pharos_engine/dynamics/'."
)
...
def resolve_softbody_class() -> Callable[..., Any]:
    ...
    1. ``pharos_engine.softbody.SoftBodyWorld`` — the WIP lattice-based
       ...
       exactly as before.
    2. ``pharos_engine.dynamics.SoftBodyWorld`` — the always-tracked
       XPBD substrate (alias for :class:`pharos_engine.dynamics.World`).
    ...
    try:
        from pharos_engine.softbody import SoftBodyWorld as _WipWorld  # type: ignore[import-not-found]
        return _WipWorld  # type: ignore[return-value]
    except Exception as exc:  # noqa: BLE001 - any import failure means "fall back"
        _LOG.debug(
            "resolve_softbody_class: pharos_engine.softbody unavailable (%s: %s); "
            "falling back to pharos_engine.dynamics.",
            type(exc).__name__, exc,
        )

    # Fallback — the always-tracked dynamics substrate.
    try:
        from pharos_engine.dynamics import SoftBodyWorld as _DynWorld
        return _DynWorld  # type: ignore[return-value]
    except Exception as exc:  # noqa: BLE001
        _LOG.debug(
            "resolve_softbody_class: pharos_engine.dynamics fallback also "
...
def _decode_body_via_dynamics(payload: dict[str, Any]) -> Any:
    ...
    Returns the decoded :class:`pharos_engine.dynamics.Body` or ``None``
    when the tracked path is unavailable (dynamics missing).
    """
    try:
        from pharos_engine.dynamics import Body, body_from_dict
...
def import_softbody_file(path: PathLike, world: Any) -> Any:
    ...
    world:
        ...
        :class:`pharos_engine.dynamics.World` or a WIP
        ``pharos_engine.softbody.SoftBodyWorld`` duck. The bridge
        prefers ``world.register_body`` when present.

    Returns
    -------
    Any
        The registered body — a :class:`pharos_engine.dynamics.Body`
