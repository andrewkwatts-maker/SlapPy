...
operate on the ``pharos_engine.visual_scripting.NodeGraph`` data model
...
_HAS_VS = False
try:
    from pharos_engine.visual_scripting import (  # type: ignore[import-not-found]
...
@dataclass
class _StubNodePort:
    """Mimic ``pharos_engine.visual_scripting.NodePort``."""
...
@dataclass
class _StubNode:
    """Mimic ``pharos_engine.visual_scripting.Node``."""
...
@dataclass
class _StubEdge:
    """Mimic ``pharos_engine.visual_scripting.Edge``."""
...
class _StubNodeGraph:
    """Mimic ``pharos_engine.visual_scripting.NodeGraph``."""
...
def _theme_accent() -> tuple[int, int, int, int]:
    ...
    try:
        from pharos_engine.ui.widgets.notebook_theme import resolve_theme
...
class NotebookNodeEditor:
    ...
    * a small icon/glyph indicating the node's kind (math = ``+``,
    ...
    def generate_python(self) -> str:
        ...
        Routes through ``pharos_engine.visual_scripting.graph_to_python``
