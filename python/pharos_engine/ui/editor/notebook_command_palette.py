...
:class:`pharos_engine.tool_router.ToolRouter` and dispatch it with a
...
by :mod:`pharos_engine.ui.editor.notebook_prefab_menu` and its cousins.

Design provenance
-----------------

* Task **CC7** — Command palette (2026-07-05 sprint).
* Router surface: :data:`pharos_engine.tool_router.REGISTRY` +
  :meth:`ToolRouter.list_actions`.
* Style reference:
  :mod:`pharos_engine.ui.editor.notebook_prefab_menu` for the
...
class NotebookCommandPalette:
    ...
    router:
        ...
        Defaults to :data:`pharos_engine.tool_router.REGISTRY` when
