...
A presentation sibling of :mod:`pharos_engine.ui.editor.spawn_menu`. Each
...
:func:`pharos_engine.ui.theme.shader_effects.noise_glitter`. Click on the
Summon button opens a modal that uses :class:`NotebookInspector` to fill
the spec fields, mirroring the Nova3D
:func:`~pharos_engine.ui.editor.spawn_menu.open_spawn_modal` contract.
...
# ---------------------------------------------------------------------------
from pharos_engine.ui.editor.notebook_spawn_menu_svgs import (
...
def _build_cards() -> list[SpawnCard]:
    ...
    :mod:`pharos_engine.ui.editor.spawn_menu`. We import them lazily so a
...
class NotebookSpawnMenu:
    ...
    def _make_shimmer_texture(self) -> Any:
        ...
        """
        try:
            from pharos_engine.ui.theme.shader_effects import noise_glitter
