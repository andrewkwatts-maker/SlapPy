...
The Rust ``pharos_engine._core`` module doesn't publish a command buffer
...
buffer — when ``pharos_engine._core.CommandBuffer`` lands we'll add a
...
def resolve_undo_stack(capacity: int = UndoStack.DEFAULT_CAPACITY) -> Any:
    ...
    Prefers ``pharos_engine._core.CommandBuffer`` when available; falls
