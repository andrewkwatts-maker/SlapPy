...
def test_direct_import_cut_and_delete() -> None:
    """Every new helper is importable from ``pharos_editor.actions``."""
    from pharos_editor.actions import cut_selection, delete_selection
...
def test_direct_import_framing_actions() -> None:
    from pharos_editor.actions import center_on_selection, frame_all
    assert center_on_selection({}) == {"status": "no_camera"}
    assert frame_all({}) == {"status": "no_camera"}


def test_direct_import_pan_tool() -> None:
    from pharos_editor.actions import PAN_TOOL_ID, activate_pan_tool
