...
introduces SlapPyEngine on first launch. Coverage:
...
class TestExports:
    def test_demo_cards_exported(self):
        from slappyengine.ui.editor.notebook_welcome import DEMO_CARDS

        assert isinstance(DEMO_CARDS, tuple)
        assert len(DEMO_CARDS) == 3

    def test_theme_swatches_exported(self):
        from slappyengine.ui.editor.notebook_welcome import THEME_SWATCHES
