...
class TestNodeDefFactories:
    ...
    def test_pixel_color_node(self):
        from pharos_engine.material.node_material import PixelColorNode
        n = PixelColorNode()
        assert n.node_type == "PixelColor"

    def test_pixel_channel_node(self):
        from pharos_engine.material.node_material import PixelChannelNode
        n = PixelChannelNode("roughness")
        assert n.node_type == "PixelChannel"
        assert n.params["channel"] == "roughness"

    def test_add_node(self):
        from pharos_engine.material.node_material import AddNode
        n = AddNode()
        assert n.node_type == "Add"

    def test_multiply_node(self):
        from pharos_engine.material.node_material import MultiplyNode
        assert MultiplyNode().node_type == "Multiply"

    def test_lerp_node(self):
        from pharos_engine.material.node_material import LerpNode
        assert LerpNode().node_type == "Lerp"

    def test_clamp_node_params(self):
        from pharos_engine.material.node_material import ClampNode
    ...
    def test_sample_sim_field_node(self):
        from pharos_engine.material.node_material import SampleSimFieldNode
    ...
    def test_sin_node(self):
        from pharos_engine.material.node_material import SinNode
        assert SinNode().node_type == "sin"

    def test_cos_node(self):
        from pharos_engine.material.node_material import CosNode
        assert CosNode().node_type == "cos"

    def test_pow_node(self):
        from pharos_engine.material.node_material import PowNode
        n = PowNode(exponent=3.0)
        assert n.node_type == "pow"
        assert n.params["exponent"] == pytest.approx(3.0)

    def test_remap_node(self):
        from pharos_engine.material.node_material import RemapNode
    ...
    def test_length_node(self):
        from pharos_engine.material.node_material import LengthNode
        assert LengthNode().node_type == "length"

    def test_normalize_node(self):
        from pharos_engine.material.node_material import NormalizeNode
        assert NormalizeNode().node_type == "normalize"

    def test_dot_node(self):
        from pharos_engine.material.node_material import DotNode
        assert DotNode().node_type == "dot"

    def test_noise_node(self):
        from pharos_engine.material.node_material import NoiseNode
    ...
    def test_world_pos_node(self):
        from pharos_engine.material.node_material import WorldPosNode
        assert WorldPosNode().node_type == "world_pos"

    def test_time_node(self):
        from pharos_engine.material.node_material import TimeNode
        assert TimeNode().node_type == "time"

    def test_offset_uv_node(self):
        from pharos_engine.material.node_material import OffsetUVNode
        assert OffsetUVNode().node_type == "offset_uv"

    def test_reflect_uv_node(self):
        from pharos_engine.material.node_material import ReflectUVNode
        assert ReflectUVNode().node_type == "reflect_uv"

    def test_accumulate_node(self):
        from pharos_engine.material.node_material import AccumulateNode
    ...
    def test_force_output_node(self):
        from pharos_engine.material.node_material import ForceOutputNode
        assert ForceOutputNode().node_type == "force_output"

    def test_reduce_output_node(self):
        from pharos_engine.material.node_material import ReduceOutputNode
...
class TestNodeMaterialBasics:
    ...
    def test_connect_adds_edge(self):
        from pharos_engine.material.node_material import NodeMaterial, UVNode, FinalColorNode
        m = NodeMaterial("test")
        uv = m.node(UVNode())
...
class TestNodeMaterialOutputMode:
    def test_final_color_node_gives_render_mode(self):
        from pharos_engine.material.node_material import NodeMaterial, FinalColorNode
        m = NodeMaterial("render_mat")
        m.node(FinalColorNode())
        assert m.output_mode == "render"

    def test_write_field_node_gives_sim_write_mode(self):
        from pharos_engine.material.node_material import NodeMaterial, WriteFieldNode
    ...
    def test_force_output_node_gives_force_mode(self):
        from pharos_engine.material.node_material import NodeMaterial, ForceOutputNode
    ...
    def test_reduce_output_node_gives_reduce_mode(self):
        from pharos_engine.material.node_material import NodeMaterial, ReduceOutputNode
