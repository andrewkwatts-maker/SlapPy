...
class TestSVGFConstants:
    def test_phi_color_positive(self):
        from pharos_engine.gi.svgf import PHI_COLOR
        assert PHI_COLOR > 0

    def test_phi_normal_positive(self):
        from pharos_engine.gi.svgf import PHI_NORMAL
        assert PHI_NORMAL > 0

    def test_phi_depth_positive(self):
        from pharos_engine.gi.svgf import PHI_DEPTH
        assert PHI_DEPTH > 0

    def test_temporal_alpha_range(self):
        from pharos_engine.gi.svgf import TEMPORAL_ALPHA
        assert 0.0 < TEMPORAL_ALPHA <= 1.0

    def test_max_history_positive(self):
        from pharos_engine.gi.svgf import MAX_HISTORY
...
class TestReSTIRConstants:
    def test_reservoir_stride_is_32(self):
        from pharos_engine.gi.restir import _RESERVOIR_STRIDE
