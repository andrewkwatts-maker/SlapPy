...
- pharos_editor.ui.editor.theme          (_rgba, color constants)
- pharos_editor.ui.editor.toolbar        (EditorToolbar pure API)
- pharos_editor.ui.editor.behavior_panel (BehaviorPanel init + _on_apply + _set_status)
- pharos_editor.ui.editor.content_browser (ContentBrowser init + set_root + ICON_COLORS)
- pharos_editor.ui.editor.layer_panel    (LayerPanel init + set_on_layer_mode_change)
- pharos_editor.ui.editor.tag_painter    (TagPainter init + mode constants)
- pharos_editor.ui.editor.ollama_setup_modal (AiOptInDialog + module constants)
...
class TestRgbaHelper:
    def _fn(self, *args, **kwargs):
        from pharos_editor.ui.editor.theme import _rgba
...
class TestAiOptInDialogInit:
    ...
    def test_selected_label_is_recommended(self):
        from pharos_editor.ui.editor.ollama_setup_modal import AiOptInDialog, AVAILABLE_MODELS
        d = AiOptInDialog()
...
class TestOllamaModuleConstants:
    def test_available_models_is_list(self):
        from pharos_editor.ui.editor.ollama_setup_modal import AVAILABLE_MODELS
    ...
    def test_model_tags_is_dict(self):
        from pharos_editor.ui.editor.ollama_setup_modal import _MODEL_TAGS
    ...
    def test_model_tags_keys_match_available(self):
        from pharos_editor.ui.editor.ollama_setup_modal import AVAILABLE_MODELS, _MODEL_TAGS
        for model in AVAILABLE_MODELS:
            assert model in _MODEL_TAGS, f"model '{model}' missing from _MODEL_TAGS"

    def test_pull_modal_tag_constant(self):
        from pharos_editor.ui.editor.ollama_setup_modal import _PULL_MODAL
        assert isinstance(_PULL_MODAL, str)
        assert len(_PULL_MODAL) > 0

    def test_opt_modal_tag_constant(self):
        from pharos_editor.ui.editor.ollama_setup_modal import _OPT_MODAL
